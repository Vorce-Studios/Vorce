                ...
                <div className="mt-3 pt-3 border-t border-slate-800/80 space-y-3 animate-in fade-in duration-200">
                  {d.rounds && d.rounds.length > 0 ? (
                    d.rounds.map((round, rIdx) => {
                      const isCeo = round.agent === 'ceo' || round.agent === 'alpha';
                      ...
                      return (
                        <div key={rIdx} className="space-y-1">
                          <div className="flex items-center justify-between text-[9px] px-1">
                            <span className={`font-semibold flex items-center gap-1.5 ${isCeo ? 'text-cyan-400' : 'text-purple-400'}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${isCeo ? 'bg-cyan-400' : 'bg-purple-400'}`}></span>
                              {isCeo ? `CEO (${providerName})` : `QA Manager (${providerName})`}
                            </span>
