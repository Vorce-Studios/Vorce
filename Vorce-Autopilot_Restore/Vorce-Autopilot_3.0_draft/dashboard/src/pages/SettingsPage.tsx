...
export default function SettingsPage({ config: propConfig, registry: propRegistry, memoryStore, onSave, onMemoryRefresh }: Props) {
  ...
  const handleDualCeoArrayChange = (key: 'ceo_chain' | 'qa_manager_chain' | 'deliberation_tasks', value: string) => {
    const list = value.split(',').map(s => s.trim()).filter(s => s.length > 0);
                ...
                <div key={phase} className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-2 bg-slate-900/40 p-3 rounded-lg border border-slate-800">
                    {rules.map((rule: any, idx: number) => (
                      <label key={rule.name} className="relative flex items-center justify-between gap-2.5 cursor-pointer select-none group">
                        <div className="flex items-center gap-2.5">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              handleNestedConfigChange('router_rules', phase, newRules);
                            }}
                            className="w-4 h-4 rounded border-slate-600 bg-slate-900 text-cyan-600 focus:ring-cyan-500/50"
                          />
                          <span className="text-xs font-medium text-slate-300 group-hover:text-white transition-colors">{rule.name}</span>
                        </div>
                        <span className="text-[9px] text-slate-600 font-mono hidden sm:inline">{rule.script}</span>
                      </label>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    ...
                    <input
                      type="text"
                      value={config.dual_ceo.ceo_chain?.join(', ') || ''}
                      onChange={(e: any) => handleDualCeoArrayChange('ceo_chain', e.target.value)}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={config.dual_ceo.qa_manager_chain?.join(', ') || ''}
                      onChange={(e: any) => handleDualCeoArrayChange('qa_manager_chain', e.target.value)}
                      className="input-field py-1.5 px-3 text-xs font-mono"
