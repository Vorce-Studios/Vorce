...
$test = New-VorceTestContext -Name 'OrchestratorDryRun'

try {
    # 1. Setze $global:VorceRoot korrekt
    $global:VorceRoot = $projectRoot
    Write-VorceTestResult -Context $test -Message "VorceRoot korrekt gesetzt" -Passed $true
    ...
    if (Test-Path $configPath) {
        ...
        Write-VorceTestResult -Context $test -Message "Config-Datei fehlt: $configPath" -Passed $false
    }
    ...
    foreach ($module in $modules) {
        if (Test-Path $module) {
            Write-VorceTestResult -Context $test -Message "Modul $module existiert" -Passed $true
        } else {
    ...
    if ($best -and $bestOverdue -gt 0) {
        Write-VorceTestResult -Context $test -Message "Select-NextMainRun gibt '$($best.Name)' zurück" -Passed $true
    } else {
        Write-VorceTestResult -Context $test -Message "Select-NextMainRun gibt $null zurück (FEHLER)" -Passed $false
    }
    ...
    if ($bestOverdue -le 0) {
        Write-VorceTestResult -Context $test -Message "Select-NextMainRun gibt $null zurück (korrekt)" -Passed $true
    } else {
        Write-VorceTestResult -Context $test -Message "Select-NextMainRun gibt Run zurück (FEHLER)" -Passed $false
    }
    ...
    foreach ($key in $expectedKeys) {
        if ($configBag.ContainsKey($key)) {
            Write-VorceTestResult -Context $test -Message "ConfigBag enthält Key: $key" -Passed $true
        } else {
            Write-VorceTestResult -Context $test -Message "ConfigBag fehlt Key: $key" -Passed $false
        }
    }
} catch {
