...
foreach ($proposal in $proposals) {
    ...
    try {
        ...
        if ($ConfigBag.DryRun) {
            ...
            $issueUrl = gh issue create --repo $repo --title $title --label "jules-task,autopilot-created" --body $body 2>&1
            if ($LASTEXITCODE -ne 0) { throw "gh issue create fehlgeschlagen: $($issueUrl | Out-String)" }
            $result = [pscustomobject]@{ html_url = ($issueUrl | Out-String).Trim(); number = $proposal.issueNumber }
        }
        ...
        if (-not $ConfigBag.DryRun) { Register-VorceQuotaUsage -AgentName "jules" -Cost 0 }
