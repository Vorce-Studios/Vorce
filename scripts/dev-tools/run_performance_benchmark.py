...
# Mapflow binary path
BINARY_PATH = "./target/release/SubI"

# Output directory for the artifacts
ARTIFACT_DIR = "artifacts/performance"

def run_benchmark():
    parser = argparse.ArgumentParser(description="SubI Performance Benchmark")
    ...
    print(f"Building SubI release binary for {binary_path}...")
    # SubI binary is usually produced by 'subi' crate
    subprocess.run(["cargo", "build", "--release", "-p", "subi"], check=True)
    ...
    print(f"Running SubI performance benchmark ({iterations} iterations, {frames} frames each)...")
    ...
    with open(txt_path, "w") as f:
        f.write("SubI Performance Benchmark Report\n")
        f.write("====================================\n\n")
