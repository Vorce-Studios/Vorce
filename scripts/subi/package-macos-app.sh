...
APP_NAME="${APP_NAME:-SubI}"
BUNDLE_ID="${SUBI_BUNDLE_ID:-info.subiteam.SubI}"
...
ICON_PATH="${ROOT_DIR}/resources/app_icons/subi.icns"
...
sed \
    -e "s/__SUBI_VERSION__/${VERSION}/g" \
    -e "s/__SUBI_BUNDLE_ID__/${BUNDLE_ID}/g" \
    "${PLIST_TEMPLATE}" > "${CONTENTS_DIR}/Info.plist"

cp "${BINARY_PATH}" "${MACOS_DIR}/${APP_NAME}"
chmod +x "${MACOS_DIR}/${APP_NAME}"
cp "${ICON_PATH}" "${RESOURCES_DIR}/subi.icns"
