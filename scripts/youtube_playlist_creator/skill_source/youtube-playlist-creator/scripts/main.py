...
import re
import os
from auth import get_authenticated_service, get_api_key_service
...
def main():
    ...
    try:
        print("Initialisiere YouTube Services...")
        read_service = get_api_key_service()
        write_service = get_authenticated_service()
        
        client = YouTubeClient(read_service, write_service)

        print(f"Lese Video-Infos für {video_id}...")
        video_info = read_service.videos().list(part="snippet", id=video_id).execute()
