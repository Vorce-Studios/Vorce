...
if not defined MPV_LIB_DIR if defined VORCE_MPV_DIR if exist "%VORCE_MPV_DIR%\lib\mpv.lib" (
    set "MPV_LIB_DIR=%VORCE_MPV_DIR%\lib"
    if exist "%VORCE_MPV_DIR%\bin" set "MPV_BIN_DIR=%VORCE_MPV_DIR%\bin"
)

if not defined MPV_LIB_DIR if defined VORCE_MPV_DIR if exist "%VORCE_MPV_DIR%\mpv.lib" (
    set "MPV_LIB_DIR=%VORCE_MPV_DIR%"
    set "MPV_BIN_DIR=%VORCE_MPV_DIR%"
)
...
if defined EXTRA_FEATURES (
    ...
    echo Starting Vorce in local release mode with default features + !EXTRA_FEATURES!...
) else (
    echo Starting Vorce in local release mode with default features...
)
...
if defined MPV_LIB_DIR (
    ...
    echo Set VORCE_MPV_DIR, LIBMPV_DIR, or MPV_DIR to a folder containing mpv.lib to enable it.
)
