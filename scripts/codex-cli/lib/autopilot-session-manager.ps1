...
$script:AutopilotTaskJournalPath = Join-Path $script:AutopilotRoot "autopilot-tasks.md"
$script:AutopilotSessionLockPath = Join-Path $script:AutopilotRoot "autopilot-session-lock.md"
$script:AutopilotTmpDir = Join-Path $script:AutopilotRoot "tmp"
$script:AutopilotToolsDir = Join-Path $script:AutopilotRoot "tools"
...
function Invoke-AutopilotCodexSession {
    ...
    if (-not (Test-Path $script:AutopilotTmpDir)) {
        New-Item -ItemType Directory -Path $script:AutopilotTmpDir -Force | Out-Null
    }
    ...
    try {
        if ($VisibleTerminal.IsPresent -or $VisibleExecTerminal.IsPresent) {
            ...
            $runnerArgs = @(
                ...
                "-Model", $Model,
                "-RepositoryRoot", (Split-Path -Parent (Split-Path -Parent $script:AutopilotRoot)),
                "-LogPath", $logPath,
