...
function Get-VorceStudiosVendorPluginTrackedPaths {
    param(
        [Parameter(Mandatory)][string]$PluginId,
        [Parameter(Mandatory)][string]$LocalPath
    )
    ...
    foreach ($relativePath in @(
        ...
        $candidate = Join-Path $LocalPath $relativePath
...
function Test-VorceStudiosVendorPluginRefreshNeeded {
    param(
        [Parameter(Mandatory)][string]$PluginId,
        [Parameter(Mandatory)][string]$LocalPath,
        [Parameter(Mandatory)][object]$InstalledPlugin
    )

    $reasons = New-Object System.Collections.Generic.List[string]

    $packagePath = [string]$InstalledPlugin.packagePath
    if ([string]::IsNullOrWhiteSpace($packagePath)) {
        $reasons.Add('missing_package_path')
    } elseif ([System.StringComparer]::OrdinalIgnoreCase.Compare($packagePath, $LocalPath) -ne 0) {
        $reasons.Add('package_path_mismatch')
    }

    if ([string]$PluginId -eq 'paperclip-plugin-github-issues') {
        $capabilities = @()
        if ($InstalledPlugin.PSObject.Properties.Name -contains 'manifestJson' -and $null -ne $InstalledPlugin.manifestJson) {
            $capabilities = @($InstalledPlugin.manifestJson.capabilities | ForEach-Object { [string]$_ })
        }

        foreach ($requiredCapability in @('companies.read', 'jobs.schedule')) {
            if ($capabilities -notcontains $requiredCapability) {
                $reasons.Add("missing_capability:{0}" -f $requiredCapability)
            }
        }
    }

    $updatedAtUtc = $null
    if ($null -ne $InstalledPlugin.updatedAt) {
        try {
            if ($InstalledPlugin.updatedAt -is [datetimeoffset]) {
                $updatedAtUtc = $InstalledPlugin.updatedAt.UtcDateTime
            } elseif ($InstalledPlugin.updatedAt -is [datetime]) {
                $updatedAtUtc = ([datetime]$InstalledPlugin.updatedAt).ToUniversalTime()
            } elseif (-not [string]::IsNullOrWhiteSpace([string]$InstalledPlugin.updatedAt)) {
                $updatedAtUtc = ([datetime]([string]$InstalledPlugin.updatedAt)).ToUniversalTime()
            }
        } catch {
            $updatedAtUtc = $null
        }
    ...
    if ($null -eq $updatedAtUtc) {
        $reasons.Add('missing_updated_at')
    } else {
        foreach ($trackedPath in @(Get-VorceStudiosVendorPluginTrackedPaths -PluginId $PluginId -LocalPath $LocalPath)) {
            try {
                $fileInfo = Get-Item -LiteralPath $trackedPath -ErrorAction Stop
                if ($fileInfo.LastWriteTimeUtc -gt $updatedAtUtc.AddSeconds(1)) {
                    $reasons.Add(("source_newer:{0}" -f [System.IO.Path]::GetFileName($trackedPath)))
                }
            } catch {
            }
        }
...
function Refresh-VorceStudiosVendorPlugin {
    ...
    $reasonText = if ($null -ne $Reasons -and @($Reasons).Count -gt 0) {
        @($Reasons) -join ', '
    } else {
        'vendor_refresh_requested'
    }

    Write-Host ("Refreshing vendor plugin {0} ({1})" -f $PluginId, $reasonText)
    ...
    if ($null -eq $reinstalled) {
        throw "Plugin '$PluginId' konnte nach dem Refresh nicht erneut installiert werden."
    }
...
function Ensure-VorceStudiosPluginInstalledFromVendor {
    ...
    if ($existing) {
        $refreshCheck = Test-VorceStudiosVendorPluginRefreshNeeded -PluginId $PluginId -LocalPath $localPath -InstalledPlugin $existing
        if ($refreshCheck.Needed) {
            try {
                return Refresh-VorceStudiosVendorPlugin -PluginId $PluginId -LocalPath $localPath -InstalledPlugin $existing -Reasons $refreshCheck.Reasons
            } catch {
                Write-Warning ("Vendor-Plugin '{0}' konnte nicht automatisch refreshed werden: {1}" -f $PluginId, $_.Exception.Message)
                return (Find-VorceStudiosPlugin -PluginId $PluginId)
            }
        }
...
function Ensure-VorceStudiosGitHubPlugin {
    ...
    if ([string]::IsNullOrWhiteSpace($ghToken)) {
        throw 'GitHub CLI ist nicht angemeldet oder liefert kein Token fuer das GitHub-Issues-Plugin.'
    }
...
function Test-VorceStudiosTelegramCredentialsAvailable {
    return (
...
function Ensure-VorceStudiosTelegramPlugin {
    ...
    if (-not (Test-VorceStudiosTelegramCredentialsAvailable)) {
        $current = Find-VorceStudiosPlugin -PluginId 'paperclip-plugin-telegram'
        if ($null -ne $current -and [string]$current.status -ne 'disabled') {
            Disable-VorceStudiosPlugin -PluginId ([string]$plugin.id) | Out-Null
        }
        return @{
            plugin = (Find-VorceStudiosPlugin -PluginId 'paperclip-plugin-telegram')
            configured = $false
            enabled = $false
    ...
    $tokenSecret = Ensure-VorceStudiosCompanySecretByName -CompanyId $Context.Company.id -Name 'Vorce Telegram Bot Token' -Value ([string]$env:VORCE_TELEGRAM_BOT_TOKEN) -Description 'Telegram bot token used by paperclip-plugin-telegram.' -RotateValue
    $config = @{
        telegramBotTokenRef = [string]$tokenSecret.id
        paperclipBaseUrl = Get-VorceStudiosApiBase
        paperclipPublicUrl = Get-VorceStudiosApiBase
        defaultChatId = [string]$env:VORCE_TELEGRAM_DEFAULT_CHAT_ID
        approvalsChatId = if (-not [string]::IsNullOrWhiteSpace([string]$env:VORCE_TELEGRAM_APPROVALS_CHAT_ID)) { [string]$env:VORCE_TELEGRAM_APPROVALS_CHAT_ID } else { [string]$env:VORCE_TELEGRAM_DEFAULT_CHAT_ID }
        errorsChatId = if (-not [string]::IsNullOrWhiteSpace([string]$env:VORCE_TELEGRAM_ERRORS_CHAT_ID)) { [string]$env:VORCE_TELEGRAM_ERRORS_CHAT_ID } else { [string]$env:VORCE_TELEGRAM_DEFAULT_CHAT_ID }
        escalationChatId = if (-not [string]::IsNullOrWhiteSpace([string]$env:VORCE_TELEGRAM_APPROVALS_CHAT_ID)) { [string]$env:VORCE_TELEGRAM_APPROVALS_CHAT_ID } else { [string]$env:VORCE_TELEGRAM_DEFAULT_CHAT_ID }
    ...
    Set-VorceStudiosPluginConfig -PluginId ([string]$plugin.id) -Config $config | Out-Null
    $current = Find-VorceStudiosPlugin -PluginId 'paperclip-plugin-telegram'
    if ($null -ne $current -and [string]$current.status -ne 'ready') {
        Enable-VorceStudiosPlugin -PluginId ([string]$plugin.id) | Out-Null
    }
...
function Connect-VorceStudiosGitHubPluginLinks {
    ...
    return @{
        ...
        note = 'GitHub-Issues-Plugin verlinkt Vorce-Issues ueber den periodischen Metadata-Backfill-Job aus vorce-studios-meta. Direkte Tool-Dispatch-Calls werden auf diesem Paperclip-Build bewusst nicht mehr forciert.'
    }
}
...
function Invoke-VorceStudiosGitHubPluginPeriodicSync {
    ...
    if ($null -eq $plugin) {
        ...
        throw 'GitHub-Issues-Plugin ist nicht installiert.'
    }
    ...
    if ($null -eq $job) {
        ...
        throw 'Periodic-Sync-Job des GitHub-Issues-Plugins wurde nicht gefunden.'
    }
    ...
    try {
        ...
        if ($IgnoreFailure.IsPresent) {
            Write-Warning ("GitHub-Plugin-Backfill konnte nicht sofort gestartet werden: {0}" -f $_.Exception.Message)
            return $null
        }
        throw
    }
