...
if ($wsEnabled -and $maxConcurrent -gt 0) {
    ...
    if ($slots -gt 0 -and @($GlobalState.working_queue).Count -gt 0) {
        ...
        foreach ($item in $toStart) {
            ...
            try {
                $cmdArgs = "-NoExit", "-File", "`"$toolsDir\run-visible-agent-task.ps1`"", "-IssueNumber", $issueNum, "-IssueTitle", "`"$issueTitle`"", "-AgentProvider", "`"$agentProvider`"", "-Repository", "`"$repo`"", "-QuotaRegistryPath", "`"$quotaRegistryPath`""
