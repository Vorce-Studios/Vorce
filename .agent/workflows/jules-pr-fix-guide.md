...
--> crates/stagegraph/src/main.rs:1099
```
...
--> crates/stagegraph/src/main.rs:26:38
```

**Lösung:**
1. Prüfe, ob das Item in `lib.rs` des Ziel-Crates exportiert wird:
   ```rust
   // crates/stagegraph-render/src/lib.rs
...
2. Oder ändere den Import-Pfad:
   ```rust
   // Falsch:
   use stagegraph_render::MeshBufferCache;

   // Richtig (wenn nicht re-exported):
   use stagegraph_render::mesh_buffer_cache::MeshBufferCache;
   ```
...
Conflicting files:
- CHANGELOG.md
- crates/stagegraph-core/src/audio/analyzer_v2.rs
```
