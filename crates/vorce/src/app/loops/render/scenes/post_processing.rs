...
use anyhow::Result;
use vorce_core::output::OutputConfig;
...
pub(crate) fn render_post_processing(
    ...
    output_config_opt: Option<&OutputConfig>,
) -> Result<()> {
    if !needs_post_processing {
        return Ok(());
    }

    let mut current_view = mesh_target_view_ref.map(|view_ref| view_ref.as_ref()).unwrap_or(view);

    if use_color_calib {
        if let (Some(color_renderer), Some(output_config)) =
            (ctx.color_calibration_renderer.as_ref(), output_config_opt)
        {
            let color_target_view =
                color_target_view_ref.map(|view_ref| view_ref.as_ref()).unwrap_or(view);
        ...
        if let Some(edge_blend_renderer) = ctx.edge_blend_renderer.as_ref() {
            ...
            *texture_bind_group = edge_blend_renderer.create_texture_bind_group(current_view);
            ...
            let config_to_use = if use_edge_blend {
                output_config_opt.map(|config| config.edge_blend.clone()).unwrap_or_default()
            } else {
