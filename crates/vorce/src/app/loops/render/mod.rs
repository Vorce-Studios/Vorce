...
pub fn render(app: &mut App, output_id: OutputId) -> Result<()> {
    ...
    if output_id == 0 {
        ...
        {
            let window_context = if let Some(ctx) = app.window_manager.get(0) {
                ctx
            } else {
                return Ok(());
            };
    ...
    if output_id == 0 {
        #[cfg(feature = "ndi")]
        {
            ...
            for vid in virtual_output_ids {
                ...
                if let Some(pid) = part_id {
                    // Check if sender exists before doing the work
                    ...
                    if !app.ndi_readbacks.contains_key(&pid) {
                        let buffer = app.backend.device.create_buffer(&wgpu::BufferDescriptor {
                            label: Some(&format!("NDI Readback Buffer {}", vid)),
                            size,
                            usage: wgpu::BufferUsages::COPY_DST | wgpu::BufferUsages::MAP_READ,
                            mapped_at_creation: false,
                        });
                    ...
                    if let Some((buffer, mapping_requested)) = app.ndi_readbacks.get_mut(&pid) {
                        ...
                        if mapping_requested.load(Ordering::SeqCst) {
                            ...
                            if !buffer.slice(..).get_mapped_range().is_empty() {
                                {
                                    let view_map = buffer.slice(..).get_mapped_range();
                                    let frame_data = view_map.to_vec();
