            ...
            for vid in virtual_output_ids {
                ...
                if let Some(pid) = part_id {
                    if let Some(sender) = app.ndi_senders.get_mut(&pid) {
                        let width = 1920;
                        let height = 1080;
                        let tex_name = format!("ndi_offscreen_{}", vid);
                        ...
                        let res = render_content(
                            RenderContext {
                                ...
                                mesh_renderer: &mut app.mesh_renderer,
                                effect_chain_renderer: &mut app.effect_chain_renderer,
                                preview_effect_chain_renderer: &mut app
                        ...
                        if let Err(e) = res {
                            tracing::error!("Offscreen render error for NDI: {}", e);
                        }

                        let mut buffer_ready = false;
                        if let Some((buffer, mapping_requested)) = app.ndi_readbacks.get_mut(&vid) {
                            if mapping_requested.load(Ordering::SeqCst) {
                                let _ = app.backend.device.poll(wgpu::PollType::Wait {
                                    submission_index: None,
                                    timeout: Some(std::time::Duration::from_millis(0)),
                                ...
                                if !buffer.slice(..).get_mapped_range().is_empty() {
                                    {
                                        let mapped_view = buffer.slice(..).get_mapped_range();
                                        let frame_data = mapped_view.to_vec();
                                        ...
                                        if let Err(e) = sender.send_frame(&video_frame) {
                                            tracing::warn!(
                                                "Failed to send NDI offscreen frame: {}",
                                                e
                                            );
                                        }
                                    }
                            ...
                            let buffer =
                                app.backend.device.create_buffer(&wgpu::BufferDescriptor {
                                    label: Some("NDI Offscreen Readback Buffer"),
                                    size,
                            ...
                            app.ndi_readbacks
                                .insert(vid, (buffer, std::sync::Arc::new(AtomicBool::new(false))));
                            buffer_ready = true;
                        }
