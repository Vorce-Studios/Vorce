...
pub(crate) fn render_post_processing(
    color_calibration_renderer: &Option<vorce_render::ColorCalibrationRenderer>,
    edge_blend_renderer: &Option<vorce_render::EdgeBlendRenderer>,
    edge_blend_texture_cache: &mut std::collections::HashMap<u64, wgpu::BindGroup>,
    edge_blend_cache: &mut std::collections::HashMap<u64, (wgpu::Buffer, wgpu::BindGroup, u64)>,
    queue: &wgpu::Queue,
    ...
    if use_color_calib {
        if let (Some(color_renderer), Some(output_config)) =
            (color_calibration_renderer.as_ref(), output_config_opt.as_ref())
        {
            let color_target_view =
                color_target_view_ref.as_ref().map(|view_ref| view_ref.as_ref()).unwrap_or(view);
    ...
    if use_edge_blend {
        if let Some(edge_blend_renderer) = edge_blend_renderer.as_ref() {
            let texture_bind_group = edge_blend_texture_cache
                .entry(output_id)
            ...
            *texture_bind_group = edge_blend_renderer.create_texture_bind_group(current_view);
            ...
            let (uniform_buffer, uniform_bind_group, last_hash) =
                edge_blend_cache.entry(output_id).or_insert_with(|| {
