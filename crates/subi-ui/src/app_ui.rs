...
use subi_control::ControlTarget;
...
pub struct AppUI {
    ...
    /// Global loop mode
    pub loop_mode: subi_media::LoopMode,
    ...
    pub discovered_hue_bridges: Vec<subi_control::hue::api::discovery::DiscoveredBridge>,
...
impl Default for AppUI {
    fn default() -> Self {
        ...
        Self {
            ...
            playback_speed: 1.0,
            loop_mode: subi_media::LoopMode::Loop,
...
impl AppUI {
            ...
            .show(ctx, |ui| {
                ...
                egui::ComboBox::from_label(self.i18n.t("label-mode"))
                    .selected_text(match self.loop_mode {
                        subi_media::LoopMode::Loop => self.i18n.t("mode-loop"),
                        subi_media::LoopMode::PlayOnce => self.i18n.t("mode-play-once"),
                    })
                    .show_ui(ui, |ui: &mut egui::Ui| {
                        if ui
                            .selectable_value(
                                &mut self.loop_mode,
                                subi_media::LoopMode::Loop,
                                self.i18n.t("mode-loop"),
                            )
                            .clicked()
                        {
                            self.actions
                                .push(UIAction::SetLoopMode(subi_media::LoopMode::Loop));
                        }
                        if ui
                            .selectable_value(
                                &mut self.loop_mode,
                                subi_media::LoopMode::PlayOnce,
                                self.i18n.t("mode-play-once"),
                            )
                            .clicked()
                        {
                            self.actions
                                .push(UIAction::SetLoopMode(subi_media::LoopMode::PlayOnce));
                        }
                    });
            });
    }
    ...
    pub fn render_inspector(
        &mut self,
        ui: &mut egui::Ui,
        module_manager: &mut subi_core::module::ModuleManager,
        layer_manager: &subi_core::LayerManager,
        output_manager: &subi_core::OutputManager,
        mapping_manager: &subi_core::MappingManager,
    ) {
    ...
    pub fn render_master_controls(
        &mut self,
        ctx: &egui::Context,
        layer_manager: &mut subi_core::LayerManager,
    ) {
    ...
    pub fn render_master_controls_embedded(
        ...
        Self::midi_learn_helper(
            ui,
            &response,
            subi_control::target::ControlTarget::MasterOpacity,
            is_learning,
        ...
        Self::midi_learn_helper(
            ui,
            &response,
            subi_control::target::ControlTarget::PlaybackSpeed(None),
            is_learning,
            last_active_element.as_ref(),
            last_active_time,
            &mut self.actions,
