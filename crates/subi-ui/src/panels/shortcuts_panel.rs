...
use subi_control::shortcuts::KeyBindings;
            ...
            for j in (i + 1)..shortcuts.len() {
                if shortcuts[i].key == shortcuts[j].key
                    ...
                    && (shortcuts[i].context == shortcuts[j].context
                        || shortcuts[i].context
                            == subi_control::shortcuts::ShortcutContext::Global
                        || shortcuts[j].context
                            == subi_control::shortcuts::ShortcutContext::Global)
                {
                ...
                .show(ui.ctx(), |ui| {
                    ...
                    if input.key_pressed(egui::Key::Escape) {
                        ...
                        if !matches!(key, egui::Key::PageUp | egui::Key::PageDown) {
                            ...
                            if let Some(subi_key) = to_subi_key(*key) {
                                new_shortcut_key =
                                    Some(Some((subi_key, to_subi_modifiers(modifiers))));
                            }
                        }
                    }
                });
...
fn to_subi_key(key: egui::Key) -> Option<subi_control::shortcuts::Key> {
    use egui::Key::*;
    use subi_control::shortcuts::Key as Mk;
...
fn to_subi_modifiers(modifiers: egui::Modifiers) -> subi_control::shortcuts::Modifiers {
    subi_control::shortcuts::Modifiers {
        ctrl: modifiers.ctrl,
        alt: modifiers.alt,
