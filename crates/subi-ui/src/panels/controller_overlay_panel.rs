...
impl ControllerOverlayPanel {
    ...
    fn message_matches_config(
        message: &MidiMessage,
        config: &subi_control::midi::MidiConfig,
    ) -> bool {
        ...
        match (message, config) {
            (
                ...
                subi_control::midi::MidiConfig::Cc {
                    channel: cfg_ch,
                    controller: cfg_cc,
                },
            ...
            (
                ...
                subi_control::midi::MidiConfig::CcRelative {
                    channel: cfg_ch,
                    controller: cfg_cc,
                },
            ) => *channel == *cfg_ch && *controller == *cfg_cc,
            (
                ...
                subi_control::midi::MidiConfig::Note {
                    channel: cfg_ch,
                    note: cfg_note,
                },
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    #[cfg(feature = "midi")]
                    {
                        ...
                        // SubI Learn
                        let subi_btn = if is_learning
                            && matches!(self.learn_target, Some(MidiLearnTarget::SubI))
                        {
                            ui.add(egui::Button::new("⏳ SubI...").fill(Color32::YELLOW))
                        } else {
                            ui.button("🎯 SubI")
                        };
                        if subi_btn.clicked() && !is_learning {
                            self.learn_target = Some(MidiLearnTarget::SubI);
        ...
        let frame_color = if self.show_assignment_colors {
            ...
            match assignment {
                Some(a) => match &a.target {
                    MidiAssignmentTarget::SubI(_) => Color32::from_rgb(0, 150, 255), // Blue
    ...
    fn show_element_list_view(&mut self, ui: &mut Ui, user_config: &mut UserConfig) {
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .selectable_label(self.element_filter == ElementFilter::SubI, "SubI")
                .clicked()
            {
                self.element_filter = ElementFilter::SubI;
            }
            if ui
                .selectable_label(
                    ...
                    if let Some(elements) = &self.elements {
                        for element in &elements.elements {
                            ...
                            let show = match self.element_filter {
                                ...
                                ElementFilter::SubI => matches!(assignment, Some(a) if matches!(a.target, MidiAssignmentTarget::SubI(_))),
