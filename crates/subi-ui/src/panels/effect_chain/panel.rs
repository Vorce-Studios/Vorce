...
impl EffectChainPanel {
    ...
    pub fn ui(
        ...
        mut recent_configs: Option<&mut subi_core::RecentEffectConfigs>,
    ) {
        if !self.visible {
            return;
        }
    ...
    fn render_toolbar(
        ...
        locale: &LocaleManager,
        icon_manager: Option<&IconManager>,
        recent_configs: &mut Option<&mut subi_core::RecentEffectConfigs>,
    ) {
        ui.horizontal(|ui| {
            // Add effect button
            if ui
                                                ...
                                                for config in configs {
                                                    if ui.button(config.name.to_string()).clone().on_hover_text(format!("{:?}", config.params)).clicked() {
                                                         ...
                                                         for (k, v) in &config.params {
                                                             if let subi_core::recent_effect_configs::EffectParamValue::Float(f) = v {
