...
pub enum MidiAssignmentTarget {
    /// Assigned to SubI internal control
    SubI(String), // Control target ID
...
pub struct UserConfig {
    ...
    /// Persisted application log level. Takes effect after restarting SubI.
    #[serde(default)]
...
impl UserConfig {
    ...
    pub fn get_assignments_by_type(
        ...
        let subi: Vec<_> = self
            .midi_assignments
            .iter()
            .filter(|a| matches!(a.target, MidiAssignmentTarget::SubI(_)))
            .collect();
...
mod tests {
    ...
    fn test_serialize_deserialize() {
        let config = UserConfig {
            ...
            last_project: Some("/path/to/project.SubI".to_string()),
