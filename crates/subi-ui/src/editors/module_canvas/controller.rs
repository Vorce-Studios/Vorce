...
use subi_core::module::{SubIModule, ModulePartId};

#[cfg(feature = "midi")]
pub fn process_midi_message(canvas: &mut ModuleCanvas, message: subi_control::midi::MidiMessage) {
    // Check if we're in learn mode for any part
    if let Some(part_id) = canvas.midi_learn_part_id {
        match message {
            subi_control::midi::MidiMessage::ControlChange {
                channel,
                controller,
                ..
            } => {
            ...
            subi_control::midi::MidiMessage::NoteOn { channel, note, .. } => {
                tracing::info!(
...
pub fn safe_delete_selection(canvas: &mut ModuleCanvas, module: &mut SubIModule) {
...
pub fn apply_undo_action(module: &mut SubIModule, action: &CanvasAction) {
    match action {
...
pub fn apply_redo_action(module: &mut SubIModule, action: &CanvasAction) {
    match action {
