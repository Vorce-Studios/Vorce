...
use subi_core::module::{ModuleId, ModuleManager, TriggerType};
        ...
        if let Some(module_id) = canvas.active_module_id {
            if let Some(module) = manager.get_module_mut(module_id) {
                if let Some(part) = module.parts.iter_mut().find(|p| p.id == part_id) {
                    if let subi_core::module::ModulePartType::Trigger(TriggerType::Midi {
                        channel: ref mut ch,
                        note: ref mut n,
                        ..
                    }) = part.part_type
        ...
        for socket_info in &all_sockets {
            if socket_info.part_id == part_id {
                ...
                if socket_resp.clicked()
                    ...
                    && socket_info.socket_type == subi_core::module::ModuleSocketType::Trigger
                {
            ...
            if let Some(pointer_pos) = ui.input(|i| i.pointer.hover_pos()) {
                ...
                if let Some(target) = closest_socket {
                    ...
                    if !exists {
                        module
                            .connections
                            .push(subi_core::module::ModuleConnection {
                                from_part: out_part,
                                from_socket: out_idx,
                                to_part: in_part,
                                to_socket: in_idx,
