    ...
    vec![
        ...
        NodeCatalogItem {
            ...
            part_type: ModulePartType::Source(SourceType::Bevy3DShape {
                shape_type: subi_core::module::BevyShapeType::Cube,
                position: [0.0, 0.0, 0.0],
                rotation: [0.0, 0.0, 0.0],
                scale: [1.0, 1.0, 1.0],
    ...
    .chain(vec![
        ...
        NodeCatalogItem {
            ...
            part_type: ModulePartType::Layer(LayerType::Single {
                ...
                opacity: 1.0,
                blend_mode: None,
                mesh: subi_core::module::MeshType::default(),
                mapping_mode: false,
            }),
        },
        NodeCatalogItem {
...
pub fn get_part_property_text(part_type: &ModulePartType) -> String {
    match part_type {
        ...
        ModulePartType::Layer(layer_type) => {
            use subi_core::module::LayerType;
            match layer_type {
...
mod tests {
    ...
    fn test_node_catalog_coverage() {
        ...
        let has_trigger = catalog.iter().any(|item| {
            matches!(
                item.part_type,
                subi_core::module::ModulePartType::Trigger(_)
            )
        });
        let has_source = catalog.iter().any(|item| {
            matches!(
                item.part_type,
                subi_core::module::ModulePartType::Source(_)
            )
        });
        let has_effect = catalog.iter().any(|item| {
            matches!(
                item.part_type,
                subi_core::module::ModulePartType::Modulizer(
                    subi_core::module::ModulizerType::Effect { .. }
                )
            )
        });
        let has_output = catalog.iter().any(|item| {
            matches!(
                item.part_type,
                subi_core::module::ModulePartType::Output(_)
            )
        });
