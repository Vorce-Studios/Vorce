...
//!
//! Parses OSC addresses like `/subi/layer/0/opacity` to control targets
...
/// - `/subi/layer/{id}/opacity` - Layer opacity (0.0-1.0)
/// - `/subi/layer/{id}/position` - Layer position (x, y)
/// - `/subi/layer/{id}/rotation` - Layer rotation (degrees)
/// - `/subi/layer/{id}/scale` - Layer scale
/// - `/subi/layer/{id}/visibility` - Layer visibility (bool)
/// - `/subi/paint/{id}/parameter/{name}` - Paint parameter
/// - `/subi/effect/{id}/parameter/{name}` - Effect parameter
/// - `/subi/playback/speed` - Playback speed
/// - `/subi/playback/position` - Playback position
/// - `/subi/output/{id}/brightness` - Output brightness
pub fn parse_osc_address(address: &str) -> Result<ControlTarget> {
    ...
    if parts.is_empty() || parts[0] != "subi" {
        return Err(ControlError::InvalidMessage(format!(
            "OSC address must start with /subi: {}",
            address
        )));
    }
...
pub fn control_target_to_address(target: &ControlTarget) -> String {
    match target {
        ControlTarget::LayerOpacity(id) => format!("/subi/layer/{}/opacity", id),
        ControlTarget::LayerPosition(id) => format!("/subi/layer/{}/position", id),
        ControlTarget::LayerScale(id) => format!("/subi/layer/{}/scale", id),
        ControlTarget::LayerRotation(id) => format!("/subi/layer/{}/rotation", id),
        ControlTarget::LayerVisibility(id) => format!("/subi/layer/{}/visibility", id),
        ControlTarget::PaintParameter(id, name) => {
            format!("/subi/paint/{}/parameter/{}", id, name)
        }
        ControlTarget::EffectParameter(id, name) => {
            format!("/subi/effect/{}/parameter/{}", id, name)
        }
        ControlTarget::PlaybackSpeed(_) => "/subi/playback/speed".to_string(),
        ControlTarget::PlaybackPosition => "/subi/playback/position".to_string(),
        ControlTarget::OutputBrightness(id) => format!("/subi/output/{}/brightness", id),
        ControlTarget::OutputEdgeBlend(id, edge) => {
            format!("/subi/output/{}/edge_blend/{:?}", id, edge)
        }
        ControlTarget::MasterOpacity => "/subi/master/opacity".to_string(),
        ControlTarget::MasterBlackout => "/subi/master/blackout".to_string(),
        ControlTarget::Custom(name) => format!("/subi/custom/{}", name),
    }
}
...
mod tests {
    ...
    fn test_parse_layer_opacity() {
        let target = parse_osc_address("/subi/layer/0/opacity").unwrap();
        assert_eq!(target, ControlTarget::LayerOpacity(0));
    }

    #[test]
    fn test_parse_layer_position() {
        let target = parse_osc_address("/subi/layer/5/position").unwrap();
        assert_eq!(target, ControlTarget::LayerPosition(5));
    }

    #[test]
    fn test_parse_paint_parameter() {
        let target = parse_osc_address("/subi/paint/3/parameter/speed").unwrap();
        assert_eq!(
            target,
    ...
    fn test_parse_effect_parameter() {
        let target = parse_osc_address("/subi/effect/1/parameter/intensity").unwrap();
        assert_eq!(
            target,
    ...
    fn test_parse_playback_speed() {
        let target = parse_osc_address("/subi/playback/speed").unwrap();
    ...
    fn test_invalid_address() {
        ...
        assert!(parse_osc_address("/subi").is_err());
        assert!(parse_osc_address("/subi/layer").is_err());
        assert!(parse_osc_address("/subi/layer/notanumber/opacity").is_err());
    }
    ...
    fn test_invalid_category() {
        assert!(parse_osc_address("/subi/unknown/test").is_err());
    }

    #[test]
    fn test_invalid_output_address() {
        assert!(parse_osc_address("/subi/output").is_err());
        assert!(parse_osc_address("/subi/output/abc").is_err());
        assert!(parse_osc_address("/subi/output/0").is_err());
        assert!(parse_osc_address("/subi/output/0/unknown").is_err());
    }

    #[test]
    fn test_invalid_master_address() {
        assert!(parse_osc_address("/subi/master").is_err());
        assert!(parse_osc_address("/subi/master/unknown").is_err());
    }
    ...
    fn test_parse_huge_address() {
        ...
        let address = format!("/subi/paint/0/parameter/{}", huge_name);
        ...
        let huge_address = format!("/subi/{}", "a".repeat(2000));
