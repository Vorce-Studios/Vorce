...
mod tests {
    ...
    fn test_sacn_sender_creation() {
        let sender = SacnSender::new(1, "SubI");
        assert!(sender.is_ok());
    }
    ...
    fn test_invalid_universe() {
        ...
        assert!(SacnSender::new(0, "SubI").is_err());

        // Universe > 63999 is invalid
        assert!(SacnSender::new(64000, "SubI").is_err());
    }

    #[test]
    fn test_sacn_packet_structure() {
        let sender = SacnSender::new(1, "SubI").unwrap();
    ...
    fn test_priority_setting() {
        let mut sender = SacnSender::new(1, "SubI").unwrap();
