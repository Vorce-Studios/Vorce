...
mod tests {
    ...
    #[test]
    fn test_osc_trigger() {
        let trigger = OscTrigger::new("/subi/cue/1".to_string());
        assert!(trigger.matches_address("/subi/cue/1"));
        assert!(!trigger.matches_address("/subi/cue/2"));
    }

    #[test]
    fn test_osc_trigger_with_value() {
        let trigger = OscTrigger::with_value("/subi/cue/1".to_string(), "1.0".to_string());
