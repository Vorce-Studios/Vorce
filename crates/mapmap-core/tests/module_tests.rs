...
#[test]
fn test_module_update_part_position_invalid_id() {
    let mut module = MapFlowModule {
        id: 1,
        name: "Test".to_string(),
        color: [1.0; 4],
    ...
    let _part_id = module.add_part(PartType::Trigger, (0.0, 0.0));

    // Attempt to update with an invalid ID
    module.update_part_position(999, (50.0, 100.0));

    // Position of the existing part should remain unchanged
    assert_eq!(module.parts[0].position, (0.0, 0.0));
}

#[test]
fn test_module_remove_connection_invalid_keeps_connections() {
    let mut module = MapFlowModule {
        id: 1,
        name: "Test".to_string(),
        color: [1.0; 4],
    ...
    module.remove_connection(999, 0, 2, 0); // Invalid from part
    module.remove_connection(1, 999, 2, 0); // Invalid from socket
    module.remove_connection(1, 0, 999, 0); // Invalid to part
    module.remove_connection(1, 0, 2, 999); // Invalid to socket
