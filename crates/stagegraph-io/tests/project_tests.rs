...
use stagegraph_core::{AppSettings, AppState};
use stagegraph_io::error::IoError;
use stagegraph_io::project::{load_project, save_project};
use std::fs::File;
use std::io::Write;
...
fn create_sample_app_state() -> AppState {
    ...
    // Or use make_mut if they are already in Arc
    {
        ...
        let layer = stagegraph_core::Layer::new(1, "Test Layer");
        layer_manager.add_layer(layer);
    }

    {
        let mapping_manager = std::sync::Arc::make_mut(&mut app_state.mapping_manager);
        let mapping = stagegraph_core::Mapping::new(1, "Test Mapping", 1, stagegraph_core::Mesh::quad());
