...
pub struct SettingsContext<'a> {
    ...
    /// The MIDI input handler.
    #[cfg(feature = "midi")]
    pub midi_handler: &'a mut Option<MidiInputHandler>,
    /// The list of available MIDI ports.
    #[cfg(feature = "midi")]
    pub midi_ports: &'a mut Vec<String>,
    /// The selected MIDI port index.
    #[cfg(feature = "midi")]
    pub selected_midi_port: &'a mut Option<usize>,
    /// Flag indicating if a restart has been requested.
    pub restart_requested: &'a mut bool,
    /// Flag indicating if an exit has been requested.
    pub exit_requested: &'a mut bool,
    /// The Tokio runtime for async tasks.
