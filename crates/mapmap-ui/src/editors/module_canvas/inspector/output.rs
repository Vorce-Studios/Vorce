...
pub fn render_output_ui(
    ...
    match output {
        OutputType::Projector {
            ...
            ui.separator();
            ui.label("⚙️ Advanced Display Settings:");
            ui.horizontal(|ui| {
                ui.label("Resolution:");
                ui.add(egui::DragValue::new(output_width).range(1..=8192).prefix("W: "));
                ui.add(egui::DragValue::new(output_height).range(1..=8192).prefix("H: "));
            });
            ui.horizontal(|ui| {
                ui.label("Refresh Rate:");
                ui.add(egui::DragValue::new(output_fps).range(1..=240).suffix(" FPS"));
            });
