...
/// Resource to store current audio analysis data from SubI.
///
...
/// Resource for sharing the rendered frame from Bevy with SubI.
///
/// This resource holds the handle to the render target texture and a shared buffer
/// containing the pixel data of the last rendered frame. This allows SubI to
...
/// Resource that maps SubI Node IDs to Bevy Entity IDs.
///
/// This allows the system to update existing Bevy entities when their corresponding
/// SubI node properties change, rather than recreating them every frame.
///
...
/// Resource that stores the current evaluated trigger values for SubI nodes.
///
...
/// Key is `(module_id, part_id)`.
#[derive(Resource, Default)]
pub struct SubITriggerResource {
