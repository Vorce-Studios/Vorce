//! Bevy integration for SubI.
//!
//! This crate integrates the Bevy engine into SubI to provide advanced 3D rendering capabilities.
//! It bridges SubI's core data structures with Bevy's ECS (Entity Component System), allowing
//! for the creation of complex 3D scenes, particle systems, and audio-reactive visuals.
//!
//! ## Features
//!
//! - **Seamless ECS Integration**: Run Bevy systems alongside SubI's main loop.
...
//! - **Shared Rendering**: Efficiently share the rendered Bevy frame with SubI's WGPU context.
//!
//! ## Usage
//!
//! This crate is primarily used by the main application to initialize the Bevy runner and update it
//! with fresh data from the SubI engine.
//!
//! ```rust,no_run
//! use subi_bevy::BevyRunner;
//!
...
/// The main entry point for running Bevy within SubI.
///
...
impl BevyRunner {
    ...
    pub fn new() -> Self {
        ...
        app.init_resource::<SubITriggerResource>();
    ...
    /// Update the Bevy simulation with new data from SubI.
    ///
    ...
    pub fn update(
        &mut self,
        audio_data: &subi_core::audio_reactive::AudioTriggerData,
        ...
        if let Some(mut res) = self
            .app
            .world_mut()
            .get_resource_mut::<crate::resources::SubITriggerResource>()
        {
    ...
    /// Update the Bevy scene based on the SubI graph state.
    ///
    /// This synchronizes the Bevy entities with the configuration of a `SubIModule`.
    /// It handles spawning new entities for new nodes and updating properties of existing ones.
    pub fn apply_graph_state(&mut self, module: &subi_core::module::SubIModule) {
        use subi_core::module::{ModulePartType, SourceType};
                        ...
                        match source_type {
                            ...
                            SourceType::BevyCamera { mode, fov, active } => {
                                    ...
                                    world.get_mut::<crate::components::BevyCamera>(entity)
                                {
                                    ...
                                    c.mode = match mode {
                                        subi_core::module::BevyCameraMode::Orbit {
                                            radius,
                                            speed,
                                            target,
                                            height,
                                        ...
                                        subi_core::module::BevyCameraMode::Fly {
                                            speed,
                                            sensitivity,
                                        ...
                                        subi_core::module::BevyCameraMode::Static {
                                            position,
                                            look_at,
