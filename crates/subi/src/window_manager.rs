...
use anyhow::Result;
use subi_core::{runtime_paths, OutputId, OutputManager};
use subi_render::WgpuBackend;
use subi_ui::config::VSyncMode;
...
impl WindowManager {
    ...
    pub fn create_output_window(
        ...
        event_loop: &ActiveEventLoop,
        backend: &WgpuBackend,
        output_config: &subi_core::OutputConfig,
    ) -> Result<()> {
        ...
        let window = Arc::new(
            event_loop.create_window(
                WindowAttributes::default()
                    .with_title(format!("SubI Output - {}", output_config.name))
                    .with_window_icon(load_app_icon())
...
fn load_app_icon() -> Option<winit::window::Icon> {
    let search_paths = [
        runtime_paths::existing_resource_path("app_icons/SubI_Logo_HQ-Full-M.png"),
        runtime_paths::existing_resource_path("app_icons/subi.png"),
    ];
