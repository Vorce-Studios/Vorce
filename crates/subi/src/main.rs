//! SubI - Open source Vj Projection Mapping Software
//!
//! This is the main application crate for SubI.
...
use subi_core::OutputId;
use subi_media::PlaybackCommand;
use subi_ui::types::MediaPlaybackCommand;
...
impl ApplicationHandler for SubIApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if self.app.is_none() {
            info!("Initializing SubI...");
            ...
            if self.is_automation {
                if let Some(fixture_path) = &self.fixture {
                    ...
                    match subi_io::load_project(std::path::Path::new(fixture_path)) {
                        Ok(loaded_state) => {
                ...
                let has_projector_outputs =
                    self.state.module_manager.modules().iter().any(|module| {
                        module.parts.iter().any(|part| {
                            matches!(
                                part.part_type,
                                subi_core::module::ModulePartType::Output(
...
fn main() -> Result<()> {
    ...
    let initial_user_config = subi_ui::config::UserConfig::load();
    let configured_log_level = match initial_user_config.log_level {
        subi_ui::config::AppLogLevel::Info => tracing::Level::INFO,
        subi_ui::config::AppLogLevel::Debug => tracing::Level::DEBUG,
    };

    // Initialize logging
    let file_appender = tracing_appender::rolling::daily("logs", "subi.log");
    ...
    info!("Starting SubI in {:?} mode...", args.mode);
    info!(
