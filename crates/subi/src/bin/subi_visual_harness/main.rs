...
use image::RgbaImage;
use subi_render::{QuadRenderer, RenderBackend, TextureDescriptor, WgpuBackend};
...
#[command(
    author,
    version,
    about = "Local visible visual regression harness for SubI"
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
...
fn render_frame(
    ...
    texture: &subi_render::TextureHandle,
    capture_output: bool,
    output_path: &Path,
) -> Result<()> {
    let frame = surface
