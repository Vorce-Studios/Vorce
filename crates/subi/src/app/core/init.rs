...
use subi_control::hue::controller::HueController;
#[cfg(feature = "midi")]
use subi_control::midi::MidiInputHandler;
use subi_control::ControlManager;
use subi_core::{
    audio::backend::{cpal_backend::CpalBackend, AudioBackend},
    media_library::MediaLibrary,
    runtime_paths, AppState, ModuleEvaluator,
};
use subi_io::load_project;
...
impl App {
    ...
    pub async fn new(
        ...
        let saved_config = subi_ui::config::UserConfig::load();
        ...
        let autosave_path =
            dirs::data_local_dir().map(|p| p.join("SubI").join("autosave.mflow"));
        ...
        if let Some(path) = &autosave_path {
            ...
            for module in state.module_manager_mut().modules_mut() {
                for part in &mut module.parts {
                    if let subi_core::module::ModulePartType::Output(
                        subi_core::module::OutputType::Projector {
                            ref mut id,
                            ref name,
                            ..
                        },
            ...
            for module in state.module_manager.modules() {
                for part in &module.parts {
                    if let subi_core::module::ModulePartType::Output(
                        subi_core::module::OutputType::Projector { id, name, .. },
                    ) = &part.part_type
                    {
            ...
            for (id, name) in missing_outputs {
                ...
                state.output_manager_mut().add_output(
                    name,
                    subi_core::output::CanvasRegion::new(0.0, 0.0, 1.0, 1.0),
                    (1920, 1080),
                );
            }
        ...
        let audio_analyzer = subi_core::audio::AudioAnalyzer::new(state.audio_config.clone());
        ...
        let preview_mesh = subi_core::Mesh {
            mesh_type: subi_core::MeshType::Quad,
            vertices: vec![
                // Top-Left (0, 0) -> UV 0,0
                subi_core::MeshVertex::new(glam::Vec2::new(0.0, 0.0), glam::Vec2::new(0.0, 0.0)),
                // Top-Right (1, 0) -> UV 1,0
                subi_core::MeshVertex::new(glam::Vec2::new(1.0, 0.0), glam::Vec2::new(1.0, 0.0)),
                // Bottom-Right (1, 1) -> UV 1,1
                subi_core::MeshVertex::new(glam::Vec2::new(1.0, 1.0), glam::Vec2::new(1.0, 1.0)),
                // Bottom-Left (0, 1) -> UV 0,1
                subi_core::MeshVertex::new(glam::Vec2::new(0.0, 1.0), glam::Vec2::new(0.0, 1.0)),
            ],
        ...
        let control_hue_conf = subi_control::hue::models::HueConfig {
        ...
        let app = Self {
            ...
            layer_ping_pong,
            state,
            history: subi_core::History::default(),
            audio_backend,
            audio_analyzer,
            audio_devices,
            egui_context,
            ...
            shader_graph_manager: subi_render::ShaderGraphManager::new(),
            recent_effect_configs: subi_core::RecentEffectConfigs::with_persistence(
                dirs::data_dir()
            ...
            bevy_runner: Some(subi_bevy::BevyRunner::new()),
        };

        // --- INITIALIZATION STATUS REPORT ---
        info!("==========================================");
        info!("   SubI Initialization Status Report   ");
        info!("------------------------------------------");
        info!(
            "- Render Backend: {} ({:?})",
