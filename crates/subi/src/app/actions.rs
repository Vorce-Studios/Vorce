...
use anyhow::Result;
use subi_io::save_project;
use subi_mcp::McpAction;
use subi_ui::{NodeEditorAction, UIAction};
use rfd::FileDialog;
        ...
        match action {
            ...
            UIAction::OpenShaderGraph(graph_id) => {
                ...
                if let Some(graph) = app.state.shader_graphs.get(&graph_id) {
                    ...
                    {
                        let new_graph = subi_core::shader_graph::ShaderGraph::new(
                            graph_id,
                            "New Graph".to_string(),
                        );
            ...
            UIAction::ResetLayout => {
                ...
                if let Some(layout) = app.ui_state.user_config.active_layout_mut() {
                    *layout = subi_ui::core::config::LayoutProfile::default_profile();
            ...
            UIAction::Play => {
                ...
                for handle in app.media_players.values_mut() {
                    let _ = handle.command_tx.send(subi_media::PlaybackCommand::Play);
                }
            }
            UIAction::Pause => {
                app.state.effect_animator_mut().pause();
                for handle in app.media_players.values_mut() {
                    let _ = handle.command_tx.send(subi_media::PlaybackCommand::Pause);
                }
            }
            UIAction::Stop => {
                app.state.effect_animator_mut().stop();
                for handle in app.media_players.values_mut() {
                    let _ = handle.command_tx.send(subi_media::PlaybackCommand::Stop);
                }
            }
            UIAction::SetSpeed(s) => {
                ...
                for handle in app.media_players.values_mut() {
                    let _ = handle
                        .command_tx
                        .send(subi_media::PlaybackCommand::SetSpeed(s));
                }
            }
            UIAction::SetLoopMode(m) => {
                for handle in app.media_players.values_mut() {
                    let _ = handle
                        .command_tx
                        .send(subi_media::PlaybackCommand::SetLoopMode(m));
                }
            }
            ...
            UIAction::Export => {
                if let Some(path) = FileDialog::new()
                    .add_filter("SubI Project Export", &["zip"])
                    .set_file_name("project_export.zip")
                    .save_file()
                {
                    if let Err(e) = subi_io::project::export_project(&app.state, &path) {
            ...
            UIAction::SaveProjectAs => {
                if let Some(path) = FileDialog::new()
                    .add_filter("SubI Project", &["mflow", "subi", "ron", "json"])
            ...
            UIAction::OpenLicense => {
                app.egui_context.open_url(egui::OpenUrl::new_tab(
                    "https://github.com/MrLongNight/SubI/blob/main/LICENSE",
                ));
            }
            ...
            UIAction::AddPaint => {
                ...
                app.state
                    .paint_manager_mut()
                    .add_paint(subi_core::Paint::color(
                        0,
            ...
            UIAction::AddMapping => {
                ...
                app.state
                    .mapping_manager_mut()
                    .add_mapping(subi_core::Mapping::quad(
                        0,
            ...
            UIAction::ConnectNdiSource { part_id, source } => {
                let receiver = app.ndi_receivers.entry(part_id).or_insert_with(|| {
                    ...
                    subi_io::ndi::NdiReceiver::new().expect("Failed to create NDI receiver")
                });
                info!(
            ...
            UIAction::SetMidiAssignment(element_id, target_id) => {
                #[cfg(feature = "midi")]
                {
                    use subi_ui::config::MidiAssignmentTarget;
                    app.ui_state.user_config.set_midi_assignment(
                        &element_id,
                        MidiAssignmentTarget::SubI(target_id.clone()),
                    );
                    tracing::info!(
            ...
            UIAction::FetchHueGroups => {
                ...
                if bridge_ip.is_empty() || username.is_empty() {
                    ...
                    let config = subi_control::hue::models::HueConfig {
                    ...
                    match app.tokio_runtime.block_on(
                        subi_control::hue::api::groups::get_entertainment_groups(&config),
                    ) {
                        Ok(groups) => {
            ...
            UIAction::ConnectHue => {
                ...
                let control_hue = subi_control::hue::models::HueConfig {
            ...
            UIAction::DiscoverHueBridges => {
                ...
                match app
                    .tokio_runtime
                    .block_on(subi_control::hue::api::discovery::discover_bridges())
                {
                    Ok(bridges) => {
            ...
            UIAction::TimelineAction(timeline_action) => {
                ...
                match timeline_action {
                    ...
                    TimelineAction::AddMarker(t) => {
                        ...
                        animator
                            .add_marker(subi_core::animation::Marker::new(id, t as f64, name));
                    }
...
pub fn handle_mcp_actions(app: &mut App) {
    while let Ok(action) = app.mcp_receiver.try_recv() {
        if let subi_mcp::McpAction::SetModuleSourcePath(mod_id, part_id, path) = action {
            info!(
            ...
            if let Some(module) = app.state.module_manager_mut().get_module_mut(mod_id) {
                if let Some(part) = module.parts.iter_mut().find(|p| p.id == part_id) {
                    if let subi_core::module::ModulePartType::Source(
                        subi_core::module::SourceType::MediaFile {
