...
fn resolve_startup_animation_source(source_path: &str) -> Option<std::path::PathBuf> {
    ...
    if let Ok(relative_to_resources) = direct_path.strip_prefix("resources") {
        if let Some(resolved) =
            subi_core::runtime_paths::existing_resource_path(relative_to_resources)
        {
            return Some(resolved);
        }
    }

    subi_core::runtime_paths::existing_resource_path(trimmed)
}
...
fn load_startup_animation(app: &mut App, source_path: &str) {
    ...
    match subi_media::open_path_with_hw_accel(&path, subi_media::HwAccelType::None) {
        Ok(mut player) => {
...
fn update_startup_animation_texture(
    ...
    let subi_io::format::FrameData::Cpu(data) = &frame.data else {
...
pub fn show(ctx: &egui::Context, app: &mut App) {
            ...
            .show(ctx, |ui_obj| {
                egui::TopBottomPanel::bottom("left_sidebar_preview")
                    ...
                    .show_inside(ui_obj, |ui_obj| {
                        ...
                        use subi_core::module::{ModulePartType, OutputType};
    ...
    subi_ui::panels::osc_panel::show_osc_panel(ctx, &mut app.ui_state, &mut app.control_manager);
