...
use super::types::TimelineModule;

/// UI state and logic for module arrangement.
...
impl TimelineState {
    pub fn sorted_enabled_blocks(&self) -> Vec<&ModuleArrangementItem> {
    ...
    pub fn sorted_enabled_block_ids(&self) -> Vec<u64> {
    ...
    pub fn find_block(&self, block_id: u64) -> Option<&ModuleArrangementItem> {
        self.module_arrangement.iter().find(|item| item.id == block_id)
    }

    pub fn first_enabled_block_id(&self) -> Option<u64> {
        self.sorted_enabled_blocks().first().map(|item| item.id)
    }

    pub fn active_block_for_time(&self, time: f32) -> Option<&ModuleArrangementItem> {
    ...
    pub fn module_name_map<'a>(modules: &[TimelineModule<'a>]) -> HashMap<ModuleId, &'a str> {
        modules.iter().map(|m| (m.id, m.name)).collect()
    }

    pub fn module_name(module_names: &HashMap<ModuleId, &str>, module_id: ModuleId) -> String {
        module_names
            .get(&module_id)
            .map(|s| s.to_string())
    ...
    pub fn reset_runtime_selection(&mut self) {
    ...
    pub fn cleanup_missing_modules(&mut self, available_module_ids: &[ModuleId]) {
    ...
    pub fn add_module_block(&mut self, module_id: ModuleId) {
        let default_start =
    ...
    pub fn set_manual_current(&mut self, block_id: Option<u64>) {
        self.manual_current_block_id = block_id;
    }

    pub fn module_for_block_id(&self, block_id: Option<u64>) -> Option<ModuleId> {
