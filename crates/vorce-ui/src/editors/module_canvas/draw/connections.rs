    ...
    F: Fn(Pos2) -> Pos2,
{
    ...
    for (conn_idx, conn) in module.connections.iter().enumerate() {
        ...
        if let (Some(from), Some(to)) = (from_part, to_part) {
            let from_socket_idx = from.outputs.iter().position(|s| s.id == conn.from_socket);
            let to_socket_idx = to.inputs.iter().position(|s| s.id == conn.to_socket);

            let socket_type = if let Some(idx) = from_socket_idx {
                &from.outputs[idx].socket_type
            } else if let Some(idx) = to_socket_idx {
                &to.inputs[idx].socket_type
            } else {
            ...
            let from_local_y = title_height
                + socket_offset_y
                + from_socket_idx.unwrap_or(0) as f32 * socket_spacing
                + socket_spacing / 2.0;
            ...
            let to_local_y = title_height
                + socket_offset_y
                + to_socket_idx.unwrap_or(0) as f32 * socket_spacing
                + socket_spacing / 2.0;
            ...
            let icon_name = match socket_type {
                vorce_core::module::ModuleSocketType::Trigger => "audio-jack1.1.svg",
                vorce_core::module::ModuleSocketType::Media => "plug.svg",
                vorce_core::module::ModuleSocketType::Effect => "usb-cable.svg",
                vorce_core::module::ModuleSocketType::Layer => "power-plug.svg",
                vorce_core::module::ModuleSocketType::Output => "audio-jack_2.svg",
                vorce_core::module::ModuleSocketType::Link => "audio-jack_1.2.svg",
            };

            let is_new_jack = icon_name == "audio-jack1.1.svg" || icon_name == "audio-jack_2.svg";
            let is_trigger = matches!(socket_type, vorce_core::module::ModuleSocketType::Trigger);
            ...
            let (stroke_width, stroke_color, glow_width) = if is_hovered {
                if alt_held {
                    ...
                    (3.0 * canvas.zoom, ui.visuals().text_color(), 8.0 * canvas.zoom)
                }
            } else {
