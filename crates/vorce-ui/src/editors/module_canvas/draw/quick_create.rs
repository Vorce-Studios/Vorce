    ...
    let filtered_items: Vec<&utils::NodeCatalogItem> = catalog
        ...
        .filter(|item| {
            if filter_lower.is_empty() {
                true
            } else if item.search_tags.contains(&filter_lower) {
                // ⚡ Bolt: Lazy evaluation (check pre-lowercased tags first)
                // avoids `label.to_lowercase()` allocations on successful matches
                true
            } else {
