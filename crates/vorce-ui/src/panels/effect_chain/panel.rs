...
pub struct EffectChainPanel {
    ...
    /// Cached lowercased preset search query
            ...
            if self.chain.effects.is_empty() {
                ...
                for idx in 0..effect_count {
                    ...
                    if new_enabled != enabled {
                        ...
                        self.actions.push(EffectChainAction::ToggleEnabled(effect_id));
                    }
                    ...
                    for (name, value) in param_changes {
                        ...
                        self.actions.push(EffectChainAction::SetParameter(effect_id, name, value));
                    }

                    if let Some(path) = new_lut_path {
                        effect.lut_path = path.clone();
                        self.actions.push(EffectChainAction::SetLUTPath(effect_id, path));
                    }
                    ...
                    .show(ui, |ui| {
                        egui::ScrollArea::vertical().max_height(200.0).show(ui, |ui| {
                            for preset in &self.presets {
                                if let Some(search) = &self.preset_search_lower {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    crate::widgets::custom::render_info_label(ui, &preset.category);
                                });
                            }
