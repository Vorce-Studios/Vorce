...
pub mod colors {
    ...
    pub const SUCCESS_COLOR: Color32 = Color32::from_rgb(0, 255, 100); // Green for success states
...
mod tests {
    ...
    fn test_theme_picker() {
        ...
        let _ = ctx.run_ui(egui::RawInput::default(), |ctx| {
            egui::CentralPanel::default().show_inside(ctx, |ui| {
                changed = theme_picker(ui, &mut theme);
            });
        });
    ...
    fn test_theme_visuals() {
        let dark = ThemeConfig::dark_visuals();
        assert!(dark.dark_mode);

        let light = ThemeConfig::light_visuals();
        assert!(!light.dark_mode);

        let hc = ThemeConfig::high_contrast_visuals();
        assert!(hc.dark_mode);

        let resolume = ThemeConfig::resolume_visuals();
        assert!(resolume.dark_mode);

        let synthwave = ThemeConfig::synthwave_visuals();
        assert!(synthwave.dark_mode);
    }
}
