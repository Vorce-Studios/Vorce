...
impl LocaleManager {
    pub fn new(lang_id: &str) -> Self {
        let lang: LanguageIdentifier = lang_id.parse().unwrap_or_else(|_| {
            "en-US".parse().unwrap_or_else(|_| panic!("Failed to parse en-US"))
        });
    ...
    fn load_bundle(lang_id: &LanguageIdentifier) -> FluentBundle<FluentResource> {
        ...
        let available_locales: Vec<LanguageIdentifier> = vec![
            "en".parse().unwrap_or_else(|_| panic!("Failed to parse")),
            "de".parse().unwrap_or_else(|_| panic!("Failed to parse")),
        ];

        // Negotiate - fluent_langneg 0.14 uses its own LanguageIdentifier
        let requested: Vec<fluent_langneg::LanguageIdentifier> =
            vec![lang_id.to_string().parse().unwrap_or_else(|_| panic!("Failed to parse"))];
        let available_fl: Vec<fluent_langneg::LanguageIdentifier> = available_locales
            .iter()
            .map(|l| l.to_string().parse().unwrap_or_else(|_| panic!("Failed to parse")))
            .collect();
        ...
        // Load resources
        let active_lang = supported.first().unwrap_or_else(|| panic!("No supported language"));
