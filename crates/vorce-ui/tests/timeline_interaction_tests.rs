...
fn test_timeline_step_manual() {
    ...
    timeline.state.manual_current_block_id = Some(1);

    // Test selection behavior via next/prev interactions
    let next_mod = timeline.state.step_manual_next();
    assert_eq!(next_mod, Some(102));
    assert_eq!(timeline.state.manual_current_block_id, Some(2));

    let prev_mod = timeline.state.step_manual_prev();
    assert_eq!(prev_mod, Some(101));
    assert_eq!(timeline.state.manual_current_block_id, Some(1));
}
...
fn test_timeline_step_semi_auto() {
    ...
    timeline.state.semi_auto_current_block_id = Some(1);
    timeline.state.semi_auto_pending_block_id = Some(2);

    // Test selection interaction: step_semi_auto_next confirms pending
    let selected_mod = timeline.state.step_semi_auto_next();
    assert_eq!(selected_mod, Some(102));
    assert_eq!(timeline.state.semi_auto_current_block_id, Some(2));
    assert_eq!(timeline.state.semi_auto_pending_block_id, None);
}
