use vorce_ui::editors::timeline_v2::{ModuleArrangementItem, ShowMode, TimelineV2, state::TimelineState};
...
fn test_timeline_fully_automated_switch() {
    ...
    let mod_id = timeline.state.runtime_show_module(timeline.show_control_enabled, timeline.show_mode, 5.0, true, &available_ids);
    assert_eq!(mod_id, Some(101));

    // Check at time 15.0 (should be module 102)
    let mod_id = timeline.state.runtime_show_module(timeline.show_control_enabled, timeline.show_mode, 15.0, true, &available_ids);
...
fn test_timeline_hybrid_mode() {
    ...
    timeline.state.hybrid_active_triggers.insert("trigA".to_string());
    let mod_id = timeline.state.runtime_show_module(timeline.show_control_enabled, timeline.show_mode, 5.0, true, &available_ids);
    assert_eq!(mod_id, Some(102));
}
