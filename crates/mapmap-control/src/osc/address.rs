...
//!
//! Parses OSC addresses like `/stagegraph/layer/0/opacity` to control targets
...
/// - `/stagegraph/layer/{id}/opacity` - Layer opacity (0.0-1.0)
/// - `/stagegraph/layer/{id}/position` - Layer position (x, y)
/// - `/stagegraph/layer/{id}/rotation` - Layer rotation (degrees)
/// - `/stagegraph/layer/{id}/scale` - Layer scale
/// - `/stagegraph/layer/{id}/visibility` - Layer visibility (bool)
/// - `/stagegraph/paint/{id}/parameter/{name}` - Paint parameter
/// - `/stagegraph/effect/{id}/parameter/{name}` - Effect parameter
/// - `/stagegraph/playback/speed` - Playback speed
/// - `/stagegraph/playback/position` - Playback position
/// - `/stagegraph/output/{id}/brightness` - Output brightness
pub fn parse_osc_address(address: &str) -> Result<ControlTarget> {
    ...
    if parts.is_empty() || parts[0] != "stagegraph" {
        return Err(ControlError::InvalidMessage(format!(
            "OSC address must start with /stagegraph: {}",
            address
        )));
    }
...
pub fn control_target_to_address(target: &ControlTarget) -> String {
    match target {
        ControlTarget::LayerOpacity(id) => format!("/stagegraph/layer/{}/opacity", id),
        ControlTarget::LayerPosition(id) => format!("/stagegraph/layer/{}/position", id),
        ControlTarget::LayerScale(id) => format!("/stagegraph/layer/{}/scale", id),
        ControlTarget::LayerRotation(id) => format!("/stagegraph/layer/{}/rotation", id),
        ControlTarget::LayerVisibility(id) => format!("/stagegraph/layer/{}/visibility", id),
        ControlTarget::PaintParameter(id, name) => {
            format!("/stagegraph/paint/{}/parameter/{}", id, name)
        }
        ControlTarget::EffectParameter(id, name) => {
            format!("/stagegraph/effect/{}/parameter/{}", id, name)
        }
        ControlTarget::PlaybackSpeed(_) => "/stagegraph/playback/speed".to_string(),
        ControlTarget::PlaybackPosition => "/stagegraph/playback/position".to_string(),
        ControlTarget::OutputBrightness(id) => format!("/stagegraph/output/{}/brightness", id),
        ControlTarget::OutputEdgeBlend(id, edge) => {
            format!("/stagegraph/output/{}/edge_blend/{:?}", id, edge)
        }
        ControlTarget::MasterOpacity => "/stagegraph/master/opacity".to_string(),
        ControlTarget::MasterBlackout => "/stagegraph/master/blackout".to_string(),
        ControlTarget::Custom(name) => format!("/stagegraph/custom/{}", name),
    }
}
...
mod tests {
    ...
    fn test_parse_layer_opacity() {
        let target = parse_osc_address("/stagegraph/layer/0/opacity").unwrap();
        assert_eq!(target, ControlTarget::LayerOpacity(0));
    }

    #[test]
    fn test_parse_layer_position() {
        let target = parse_osc_address("/stagegraph/layer/5/position").unwrap();
        assert_eq!(target, ControlTarget::LayerPosition(5));
    }

    #[test]
    fn test_parse_paint_parameter() {
        let target = parse_osc_address("/stagegraph/paint/3/parameter/speed").unwrap();
        assert_eq!(
            target,
    ...
    fn test_parse_effect_parameter() {
        let target = parse_osc_address("/stagegraph/effect/1/parameter/intensity").unwrap();
        assert_eq!(
            target,
    ...
    fn test_parse_playback_speed() {
        let target = parse_osc_address("/stagegraph/playback/speed").unwrap();
    ...
    fn test_invalid_address() {
        ...
        assert!(parse_osc_address("/stagegraph").is_err());
        assert!(parse_osc_address("/stagegraph/layer").is_err());
        assert!(parse_osc_address("/stagegraph/layer/notanumber/opacity").is_err());
    }
    ...
    fn test_invalid_category() {
        assert!(parse_osc_address("/stagegraph/unknown/test").is_err());
    }

    #[test]
    fn test_invalid_output_address() {
        assert!(parse_osc_address("/stagegraph/output").is_err());
        assert!(parse_osc_address("/stagegraph/output/abc").is_err());
        assert!(parse_osc_address("/stagegraph/output/0").is_err());
        assert!(parse_osc_address("/stagegraph/output/0/unknown").is_err());
    }

    #[test]
    fn test_invalid_master_address() {
        assert!(parse_osc_address("/stagegraph/master").is_err());
        assert!(parse_osc_address("/stagegraph/master/unknown").is_err());
    }
    ...
    fn test_parse_huge_address() {
        ...
        let address = format!("/stagegraph/paint/0/parameter/{}", huge_name);
        ...
        let huge_address = format!("/stagegraph/{}", "a".repeat(2000));
