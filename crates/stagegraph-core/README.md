...
`stagegraph-core` is the heart of MapFlow, containing the fundamental data structures, state management, and business logic that drives the application.
...
This crate is primarily used by `stagegraph` (the application binary) and `stagegraph-ui` (for data visualization).

```rust
use stagegraph_core::Project;
