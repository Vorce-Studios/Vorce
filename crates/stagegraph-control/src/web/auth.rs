    ...
    if let Some(ws_protocol_header) = headers.get(http::header::SEC_WEBSOCKET_PROTOCOL) {
        if let Ok(protocols) = ws_protocol_header.to_str() {
            for protocol in protocols.split(',') {
                ...
                if let Some(token) = protocol.strip_prefix("stagegraph.auth.") {
                    return Some(token.to_string());
                }
            }
        }
...
mod tests {
    ...
    fn test_extract_websocket_protocol() {
        let mut headers = http::HeaderMap::new();
        // Sec-WebSocket-Protocol: stagegraph.auth.<TOKEN>
        headers.insert(
            http::header::SEC_WEBSOCKET_PROTOCOL,
            "stagegraph.auth.test_key, json".parse().unwrap(),
        );
