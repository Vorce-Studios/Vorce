...
impl MidiMessage {
        ...
        match message_type {
            0x90 => {
                ...
                if velocity == 0 {
                    ...
                    Some(MidiMessage::NoteOn { channel, note: bytes[1], velocity })
                }
            }
            0x80 => {
                // Note Off
            ...
            0xB0 => {
                // Control Change
                Some(MidiMessage::ControlChange { channel, controller: bytes[1], value: bytes[2] })
            }
            0xC0 => {
                // Program Change
                Some(MidiMessage::ProgramChange { channel, program: bytes[1] })
            }
            0xE0 => {
                // Pitch Bend
    ...
    pub fn matches(&self, other: &MidiMessage) -> bool {
        match (self, other) {
            (
                MidiMessage::NoteOn { channel: ch1, note: n1, .. },
                MidiMessage::NoteOn { channel: ch2, note: n2, .. },
            ) => ch1 == ch2 && n1 == n2,
            (
                MidiMessage::ControlChange { channel: ch1, controller: c1, .. },
                MidiMessage::ControlChange { channel: ch2, controller: c2, .. },
            ) => ch1 == ch2 && c1 == c2,
            (
...
mod tests {
    ...
    fn test_midi_message_parsing() {
        ...
        assert_eq!(msg, Some(MidiMessage::ControlChange { channel: 0, controller: 7, value: 64 }));
    ...
    fn test_midi_message_parsing_extended() {
        ...
        assert_eq!(msg, Some(MidiMessage::ProgramChange { channel: 0, program: 10 }));
    ...
    fn test_midi_matches() {
        // Note On Matches
        let n1 = MidiMessage::NoteOn { channel: 1, note: 60, velocity: 100 };
        let n2 = MidiMessage::NoteOn { channel: 1, note: 60, velocity: 50 }; // Different velocity
        let n3 = MidiMessage::NoteOn { channel: 1, note: 61, velocity: 100 }; // Different note

        assert!(n1.matches(&n2));
        assert!(!n1.matches(&n3));

        // CC Matches
        let c1 = MidiMessage::ControlChange { channel: 2, controller: 10, value: 0 };
        let c2 = MidiMessage::ControlChange { channel: 2, controller: 10, value: 127 };
        let c3 = MidiMessage::ControlChange { channel: 2, controller: 11, value: 0 };

        assert!(c1.matches(&c2));
        assert!(!c1.matches(&c3));

        // Pitch Bend Matches (channel only)
        let p1 = MidiMessage::PitchBend { channel: 3, value: 0 };
        let p2 = MidiMessage::PitchBend { channel: 3, value: 16383 };
        let p3 = MidiMessage::PitchBend { channel: 4, value: 0 };
