...
use stagegraph_core::module::{MapFlowModule, ModulePartId};

#[cfg(feature = "midi")]
pub fn process_midi_message(canvas: &mut ModuleCanvas, message: stagegraph_control::midi::MidiMessage) {
    // Check if we're in learn mode for any part
    if let Some(part_id) = canvas.midi_learn_part_id {
        match message {
            stagegraph_control::midi::MidiMessage::ControlChange {
                channel,
                controller,
                ..
            } => {
            ...
            stagegraph_control::midi::MidiMessage::NoteOn { channel, note, .. } => {
                tracing::info!(
