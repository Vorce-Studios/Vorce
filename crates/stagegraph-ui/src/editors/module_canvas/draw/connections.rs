...
use stagegraph_core::module::MapFlowModule;
    ...
    for (conn_idx, conn) in module.connections.iter().enumerate() {
        ...
        if let (Some(from), Some(to)) = (from_part, to_part) {
            let socket_type = if let Some(socket) = from.outputs.get(conn.from_socket) {
                ...
                &stagegraph_core::module::ModuleSocketType::Media
            };
            ...
            let icon_name = match socket_type {
                stagegraph_core::module::ModuleSocketType::Trigger => "audio-jack1.1.svg",
                stagegraph_core::module::ModuleSocketType::Media => "plug.svg",
                stagegraph_core::module::ModuleSocketType::Effect => "usb-cable.svg",
                stagegraph_core::module::ModuleSocketType::Layer => "power-plug.svg",
                stagegraph_core::module::ModuleSocketType::Output => "audio-jack_2.svg",
                stagegraph_core::module::ModuleSocketType::Link => "audio-jack_1.2.svg",
            };

            let is_new_jack = icon_name == "audio-jack1.1.svg" || icon_name == "audio-jack_2.svg";
            let is_trigger = matches!(socket_type, stagegraph_core::module::ModuleSocketType::Trigger);
