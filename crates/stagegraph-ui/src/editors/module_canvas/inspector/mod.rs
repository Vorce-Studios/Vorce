        ...
        .show(ui, |ui| {
            ...
            match &mut part.part_type {
                    ...
                    match trigger {
                        ...
                        TriggerType::Midi { channel, note, device: _ } => {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                {
                                    if let Ok(ports) =
                                        stagegraph_control::midi::MidiInputHandler::list_ports()
                                    {
                                        if ports.is_empty() {
                        ...
                        SourceType::Bevy3DShape { shape_type, position, rotation, scale, color, unlit, outline_width, outline_color, .. } => {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                egui::ComboBox::from_id_salt("shape_type_select").selected_text(format!("{:?}", shape_type)).show_ui(ui, |ui| {
                                    ui.selectable_value(shape_type, stagegraph_core::module::BevyShapeType::Cube, "Cube");
                                    ui.selectable_value(shape_type, stagegraph_core::module::BevyShapeType::Sphere, "Sphere");
                                    ui.selectable_value(shape_type, stagegraph_core::module::BevyShapeType::Capsule, "Capsule");
                                    ui.selectable_value(shape_type, stagegraph_core::module::BevyShapeType::Torus, "Torus");
                                    ui.selectable_value(shape_type, stagegraph_core::module::BevyShapeType::Cylinder, "Cylinder");
                                    ui.selectable_value(shape_type, stagegraph_core::module::BevyShapeType::Plane, "Plane");
                                });
                            });
                            ui.horizontal(|ui| { ui.label("Color:"); ui.color_edit_button_rgba_unmultiplied(color); });
                ...
                ModulePartType::Layer(layer) => {
                    ...
                    ui.label("📋 Layer:");
                    let mut render_mesh_ui = |ui: &mut Ui, mesh: &mut stagegraph_core::module::MeshType, id_salt: u64| { mesh::render_mesh_editor_ui(mesh_editor, last_mesh_edit_id, ui, mesh, part_id, id_salt); };
                    match layer {
                    ...
                    match output {
                        ...
                        OutputType::Hue { bridge_ip, username, client_key: _client_key, entertainment_area, lamp_positions, mapping_mode } => {
                            ...
                            ui.collapsing("⚙️ Setup (Bridge & Pairing)", |ui| {
                                ...
                                if let Some(rx) = &canvas.hue_discovery_rx { if let Ok(result) = rx.try_recv() { canvas.hue_discovery_rx = None; let result: Result<Vec<stagegraph_control::hue::api::discovery::DiscoveredBridge>, _> = result; match result { Ok(bridges) => { canvas.hue_bridges = bridges; canvas.hue_status_message = Some(format!("Found {} bridges", canvas.hue_bridges.len())); } Err(e) => { canvas.hue_status_message = Some(format!("Discovery failed: {}", e)); } } } else { ui.horizontal(|ui| { ui.spinner(); ui.label("Searching for bridges..."); }); } }
...
fn render_hue_bridge_discovery(canvas: &mut ModuleCanvas, ui: &mut Ui, current_ip: &mut String) {
    if ui.button("🔍 Discover Bridges").clicked() {
        ...
        #[cfg(feature = "tokio")]
        {
            ...
            let task = async move {
                let result = stagegraph_control::hue::api::discovery::discover_bridges()
                    .await
            ...
            for (idx, socket) in part.inputs.iter().enumerate() {
                ui.push_id(idx, |ui| {
                    ...
                    egui::ComboBox::from_id_salt("target")
                        ...
                        .show_ui(ui, |ui| {
                            use stagegraph_core::module::TriggerTarget;
                    ...
                    if config.target != TriggerTarget::None {
                        ui.horizontal(|ui| {
                            ...
                            egui::ComboBox::from_id_salt("mode")
                                ...
                                .show_ui(ui, |ui| {
                                    use stagegraph_core::module::TriggerMappingMode;
                                    ui.selectable_value(
                                        &mut config.mode,
