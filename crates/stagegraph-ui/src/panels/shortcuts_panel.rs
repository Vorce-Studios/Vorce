...
use stagegraph_control::shortcuts::KeyBindings;
            ...
            for j in (i + 1)..shortcuts.len() {
                if shortcuts[i].key == shortcuts[j].key
                    ...
                    && (shortcuts[i].context == shortcuts[j].context
                        || shortcuts[i].context
                            == stagegraph_control::shortcuts::ShortcutContext::Global
                        || shortcuts[j].context
                            == stagegraph_control::shortcuts::ShortcutContext::Global)
                {
                ...
                .show(ui.ctx(), |ui| {
                    ...
                    if input.key_pressed(egui::Key::Escape) {
                        ...
                        if !matches!(key, egui::Key::PageUp | egui::Key::PageDown) {
                            ...
                            if let Some(stagegraph_key) = to_stagegraph_key(*key) {
                                new_shortcut_key =
                                    Some(Some((stagegraph_key, to_stagegraph_modifiers(modifiers))));
                            }
                        }
                    }
                });
...
fn to_stagegraph_key(key: egui::Key) -> Option<stagegraph_control::shortcuts::Key> {
    use egui::Key::*;
    use stagegraph_control::shortcuts::Key as Mk;
...
fn to_stagegraph_modifiers(modifiers: egui::Modifiers) -> stagegraph_control::shortcuts::Modifiers {
    stagegraph_control::shortcuts::Modifiers {
        ctrl: modifiers.ctrl,
        alt: modifiers.alt,
