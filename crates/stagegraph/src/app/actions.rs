...
use anyhow::Result;
use stagegraph_io::save_project;
use stagegraph_mcp::McpAction;
use stagegraph_ui::{NodeEditorAction, UIAction};
use rfd::FileDialog;
        ...
        match action {
            ...
            UIAction::OpenShaderGraph(graph_id) => {
                ...
                if let Some(graph) = app.state.shader_graphs.get(&graph_id) {
                    ...
                    {
                        let new_graph = stagegraph_core::shader_graph::ShaderGraph::new(
                            graph_id,
                            "New Graph".to_string(),
                        );
            ...
            UIAction::ResetLayout => {
                ...
                if let Some(layout) = app.ui_state.user_config.active_layout_mut() {
                    *layout = stagegraph_ui::core::config::LayoutProfile::default_profile();
            ...
            UIAction::Play => {
                ...
                for handle in app.media_players.values_mut() {
                    let _ = handle.command_tx.send(stagegraph_media::PlaybackCommand::Play);
                }
            }
            UIAction::Pause => {
                app.state.effect_animator_mut().pause();
                for handle in app.media_players.values_mut() {
                    let _ = handle.command_tx.send(stagegraph_media::PlaybackCommand::Pause);
                }
            }
            UIAction::Stop => {
                app.state.effect_animator_mut().stop();
                for handle in app.media_players.values_mut() {
                    let _ = handle.command_tx.send(stagegraph_media::PlaybackCommand::Stop);
                }
            }
            UIAction::SetSpeed(s) => {
                ...
                for handle in app.media_players.values_mut() {
                    let _ = handle
                        .command_tx
                        .send(stagegraph_media::PlaybackCommand::SetSpeed(s));
                }
            }
            UIAction::SetLoopMode(m) => {
                for handle in app.media_players.values_mut() {
                    let _ = handle
                        .command_tx
                        .send(stagegraph_media::PlaybackCommand::SetLoopMode(m));
                }
            }
            ...
            UIAction::Export => {
                if let Some(path) = FileDialog::new()
                    ...
                    .save_file()
                {
                    if let Err(e) = stagegraph_io::project::export_project(&app.state, &path) {
            ...
            UIAction::SaveProjectAs => {
                if let Some(path) = FileDialog::new()
                    .add_filter("MapFlow Project", &["mflow", "stagegraph", "ron", "json"])
            ...
            UIAction::AddPaint => {
                ...
                app.state
                    .paint_manager_mut()
                    .add_paint(stagegraph_core::Paint::color(
                        0,
            ...
            UIAction::AddMapping => {
                ...
                app.state
                    .mapping_manager_mut()
                    .add_mapping(stagegraph_core::Mapping::quad(
                        0,
            ...
            UIAction::ConnectNdiSource { part_id, source } => {
                let receiver = app.ndi_receivers.entry(part_id).or_insert_with(|| {
                    ...
                    stagegraph_io::ndi::NdiReceiver::new().expect("Failed to create NDI receiver")
                });
                info!(
            ...
            UIAction::SetMidiAssignment(element_id, target_id) => {
                #[cfg(feature = "midi")]
                {
                    use stagegraph_ui::config::MidiAssignmentTarget;
            ...
            UIAction::FetchHueGroups => {
                ...
                if bridge_ip.is_empty() || username.is_empty() {
                    ...
                    let config = stagegraph_control::hue::models::HueConfig {
                    ...
                    match app.tokio_runtime.block_on(
                        stagegraph_control::hue::api::groups::get_entertainment_groups(&config),
                    ) {
                        Ok(groups) => {
            ...
            UIAction::ConnectHue => {
                ...
                let control_hue = stagegraph_control::hue::models::HueConfig {
            ...
            UIAction::DiscoverHueBridges => {
                ...
                match app
                    .tokio_runtime
                    .block_on(stagegraph_control::hue::api::discovery::discover_bridges())
                {
                    Ok(bridges) => {
            ...
            UIAction::TimelineAction(timeline_action) => {
                use stagegraph_ui::TimelineAction;
                match timeline_action {
                    ...
                    TimelineAction::AddMarker(t) => {
                        ...
                        animator.add_marker(stagegraph_core::animation::Marker::new(id, t as f64, name));
                    }
...
pub fn handle_mcp_actions(app: &mut App) {
    while let Ok(action) = app.mcp_receiver.try_recv() {
        if let stagegraph_mcp::McpAction::SetModuleSourcePath(mod_id, part_id, path) = action {
            info!(
