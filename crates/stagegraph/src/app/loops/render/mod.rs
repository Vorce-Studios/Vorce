...
pub fn render(app: &mut App, output_id: OutputId) -> Result<()> {
    ...
    if output_id == 0 {
        ...
        if let Some(runner) = &app.bevy_runner {
            let runner: &stagegraph_bevy::BevyRunner = runner;
    ...
    // --- RENDER PASS ---
    {
        ...
        #[cfg(feature = "ndi")]
        {
            ...
            let part_id = app.render_ops.iter().find_map(|(_, op)| {
                if let stagegraph_core::module::OutputType::Projector { id, .. } = &op.output_type {
                    if *id == output_id {
                            ...
                            if !buffer.slice(..).get_mapped_range().is_empty() {
                                {
                                    ...
                                    let video_frame = stagegraph_io::format::VideoFrame {
                                        data: stagegraph_io::format::FrameData::Cpu(Arc::new(
                                            frame_data,
                                        )),
                                        format: stagegraph_io::format::VideoFormat {
                                            width,
                                            height,
                                            pixel_format: stagegraph_io::format::PixelFormat::BGRA8,
                                            frame_rate: 60.0,
                                        },
