...
use stagegraph_control::hue::controller::HueController;
#[cfg(feature = "midi")]
use stagegraph_control::midi::MidiInputHandler;
use stagegraph_control::ControlManager;
use stagegraph_core::{
    audio::backend::{cpal_backend::CpalBackend, AudioBackend},
    media_library::MediaLibrary,
    runtime_paths, AppState, ModuleEvaluator,
};
use stagegraph_io::load_project;
use stagegraph_mcp::McpServer;
use stagegraph_render::{
...
impl App {
    ...
    pub async fn new(
        ...
        let saved_config = stagegraph_ui::config::UserConfig::load();
        ...
        if let Some(path) = &autosave_path {
            ...
            for module in state.module_manager_mut().modules_mut() {
                for part in &mut module.parts {
                    if let stagegraph_core::module::ModulePartType::Output(
                        stagegraph_core::module::OutputType::Projector {
                            ref mut id,
                            ref name,
                            ..
                        },
            ...
            for module in state.module_manager.modules() {
                for part in &module.parts {
                    if let stagegraph_core::module::ModulePartType::Output(
                        stagegraph_core::module::OutputType::Projector { id, name, .. },
                    ) = &part.part_type
                    {
            ...
            for (id, name) in missing_outputs {
                ...
                state.output_manager_mut().add_output(
                    name,
                    stagegraph_core::output::CanvasRegion::new(0.0, 0.0, 1.0, 1.0),
                    (1920, 1080),
                );
            }
        ...
        let audio_analyzer = stagegraph_core::audio::AudioAnalyzer::new(state.audio_config.clone());
        ...
        let preview_mesh = stagegraph_core::Mesh {
            mesh_type: stagegraph_core::MeshType::Quad,
            vertices: vec![
                // Top-Left (0, 0) -> UV 0,0
                stagegraph_core::MeshVertex::new(glam::Vec2::new(0.0, 0.0), glam::Vec2::new(0.0, 0.0)),
                // Top-Right (1, 0) -> UV 1,0
                stagegraph_core::MeshVertex::new(glam::Vec2::new(1.0, 0.0), glam::Vec2::new(1.0, 0.0)),
                // Bottom-Right (1, 1) -> UV 1,1
                stagegraph_core::MeshVertex::new(glam::Vec2::new(1.0, 1.0), glam::Vec2::new(1.0, 1.0)),
                // Bottom-Left (0, 1) -> UV 0,1
                stagegraph_core::MeshVertex::new(glam::Vec2::new(0.0, 1.0), glam::Vec2::new(0.0, 1.0)),
            ],
        ...
        let control_hue_conf = stagegraph_control::hue::models::HueConfig {
        ...
        let app = Self {
            ...
            layer_ping_pong,
            state,
            history: stagegraph_core::History::default(),
            audio_backend,
            audio_analyzer,
            audio_devices,
            egui_context,
            ...
            shader_graph_manager: stagegraph_render::ShaderGraphManager::new(),
            recent_effect_configs: stagegraph_core::RecentEffectConfigs::with_persistence(
                dirs::data_dir()
            ...
            bevy_runner: Some(stagegraph_bevy::BevyRunner::new()),
        };
