...
use stagegraph_core::OutputId;
use stagegraph_media::PlaybackCommand;
use stagegraph_ui::types::MediaPlaybackCommand;
        ...
        if self.app.is_none() {
            ...
            if self.is_automation {
                if let Some(fixture_path) = &self.fixture {
                    ...
                    match stagegraph_io::load_project(std::path::Path::new(fixture_path)) {
                        Ok(loaded_state) => {
                ...
                let has_projector_outputs =
                    self.state.module_manager.modules().iter().any(|module| {
                        module.parts.iter().any(|part| {
                            matches!(
                                part.part_type,
                                stagegraph_core::module::ModulePartType::Output(
...
fn main() -> Result<()> {
    ...
    let initial_user_config = stagegraph_ui::config::UserConfig::load();
    let configured_log_level = match initial_user_config.log_level {
        stagegraph_ui::config::AppLogLevel::Info => tracing::Level::INFO,
        stagegraph_ui::config::AppLogLevel::Debug => tracing::Level::DEBUG,
    };
