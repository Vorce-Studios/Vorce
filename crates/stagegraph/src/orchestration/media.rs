...
use stagegraph_render::TexturePool;
...
pub struct MediaPlayerHandle {
    ...
    pub command_tx: Sender<stagegraph_media::PlaybackCommand>,
...
pub fn create_player_handle(
    ...
    let mut player = stagegraph_media::open_path(&path_buf).map_err(anyhow::Error::from)?;
    player
    ...
    if let Some(frame) = player.update(Duration::ZERO) {
        if let stagegraph_io::format::FrameData::Cpu(ref data) = frame.data {
