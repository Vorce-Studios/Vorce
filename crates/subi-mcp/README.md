...
The **Model Context Protocol (MCP)** server for SubI.
This crate enables AI assistants (like Claude, Gemini, or custom agents) to interact with and control the SubI application.

## Overview

SubI MCP exposes the internal state and control surface of the application via the standard [Model Context Protocol](https://modelcontextprotocol.io/).
This allows for:
...
The MCP Server runs as a background service within the main SubI application (or as a standalone process for testing).
...
This crate is primarily used internally by `subi-control` and the main `subi` binary.
