...
use subi_control::osc::client::OscClient;
...
impl McpServer {
    pub fn new(action_sender: Option<crossbeam_channel::Sender<crate::McpAction>>) -> Self {
        // Try to connect to default SubI OSC port
            ...
            "resources/list" => {
                let resources = vec![
                    serde_json::json!({
                        ...
                        "description": "The current SubI project state"
                    }),
                    serde_json::json!({
                        "uri": "layer://list",
                        "name": "Layer List",
...
mod tests {
    ...
    async fn test_handle_project_save_load() {
        ...
        let action = rx.try_recv().unwrap();
        match action {
            McpAction::SaveProject(path) => assert_eq!(path.to_str().unwrap(), "test.subi"),
        ...
        let action = rx.try_recv().unwrap();
        match action {
            McpAction::LoadProject(path) => assert_eq!(path.to_str().unwrap(), "other.subi"),
    ...
    async fn test_security_path_traversal() {
        ...
        match valid_action {
            McpAction::SaveProject(path) => {
                assert_eq!(path.to_str().unwrap(), "good_project.subi")
            }
            other => panic!("Expected SaveProject action, got {:?}", other),
        }
    }
