...
fn test_layer_manager_crud() {
    let mut manager = subi_core::LayerManager::new();
...
fn test_composition_new_default_initializes_correctly() {
    let comp = subi_core::layer::Composition::default();
...
fn test_set_master_opacity_out_of_bounds_clamps_value() {
    let mut comp = subi_core::layer::Composition::default();
...
fn test_layer_builder_methods_sets_properties_correctly() {
    let layer = Layer::new(1, "Test Layer")
        .with_paint(42)
        .with_blend_mode(subi_core::layer::BlendMode::Multiply)
        .with_opacity(0.8);

    assert_eq!(layer.id, 1);
    assert_eq!(layer.name, "Test Layer");
    assert_eq!(layer.paint_id, Some(42));
    assert_eq!(layer.blend_mode, subi_core::layer::BlendMode::Multiply);
