use subi_core::module::{
    HueMappingMode, SubIModule, ModuleManager, ModulePartType, ModulePlaybackMode,
...
fn test_effect_type_all_and_name() {
    let all = subi_core::module::EffectType::all();
    assert!(!all.is_empty());
    ...
    let sg = subi_core::module::EffectType::ShaderGraph(1);
    assert_eq!(sg.name(), "Custom Shader Graph");
}

#[test]
fn test_blend_mode_type_all_and_name() {
    let all = subi_core::module::BlendModeType::all();
    assert!(!all.is_empty());
...
fn test_mesh_type_to_mesh_and_hash() {
    let mesh_type = subi_core::module::MeshType::Grid { cols: 2, rows: 2 };
...
fn test_shared_media() {
    let mut media = subi_core::module::SharedMediaState::new();
    media.register(
        "id1".to_string(),
        "path1".to_string(),
        subi_core::module::SharedMediaType::Video,
    );

    let item = media.get("id1").unwrap();
    assert_eq!(item.path, "path1");
    assert_eq!(item.media_type, subi_core::module::SharedMediaType::Video);
