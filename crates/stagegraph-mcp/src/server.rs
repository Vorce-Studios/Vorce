...
use stagegraph_control::osc::client::OscClient;
...
mod tests {
    ...
    async fn test_handle_project_save_load() {
        ...
        let action = rx.try_recv().unwrap();
        match action {
            McpAction::SaveProject(path) => assert_eq!(path.to_str().unwrap(), "test.stagegraph"),
        ...
        let action = rx.try_recv().unwrap();
        match action {
            McpAction::LoadProject(path) => assert_eq!(path.to_str().unwrap(), "other.stagegraph"),
    ...
    async fn test_security_path_traversal() {
        ...
        match valid_action {
            McpAction::SaveProject(path) => {
                assert_eq!(path.to_str().unwrap(), "good_project.stagegraph")
            }
            other => panic!("Expected SaveProject action, got {:?}", other),
        }
    }
