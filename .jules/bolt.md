...
## 2025-01-01 - Avoid cloning entire Pointer state in egui
**Erkenntnis:** In egui-Anwendungen kann das Klonen großer Zustandsstrukturen wie `ui.ctx().input(|i| i.pointer.clone())` pro Frame zu signifikanten unnötigen Allokationen führen. Dies ist besonders kritisch in Immediate Mode GUIs, wo dieser Code bis zu 60 Mal pro Sekunde ausgeführt wird.
**Aktion:** Nutze Stattdessen Closures, um nur die spezifischen Boolean-Werte abzufragen, z.B. `ui.ctx().input(|i| (i.pointer.any_released(), i.pointer.any_click(), i.pointer.primary_down()))`.
...
## 2025-01-30 - [Zero-Allocation Extension Check]
**Erkenntnis:** In Render-Loops und Directory-Scans verursachen String-Allokationen durch `.to_lowercase()` bei Media File Checks vermeidbaren Heap-Overhead.
**Aktion:** Für Case-Insensitive Checks `eq_ignore_ascii_case` direkt auf den Slices verwenden.
...
## 2024-05-18 - [Vermeidung von String Allokationen in File-Extensions]
**Erkenntnis:** Das Prüfen von File-Extensions mittels .to_lowercase() innerhalb von Render-Loops oder File-System-Scans erzeugt unnötige Heap-Allokationen (String) und bremst die Performance.
**Aktion:** Nutze eq_ignore_ascii_case, um allokationsfrei direkt auf &str (Slices) oder Iterator-Streams zu vergleichen.
