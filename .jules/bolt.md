...
**Erkenntnis:** Calling `.to_lowercase()` inside high-frequency UI rendering loops (like in the search and quick create popups) generates unnecessary heap allocations on every frame when the search query is empty or remains unchanged.
**Aktion:** I cached the lowercased search queries in `ModuleCanvas::search_filter_lower` and `ModuleCanvas::quick_create_filter_lower` (`Option<String>`) and only updated them when `egui::TextEdit::changed()` returns true. This eliminated the allocation overhead completely during idle rendering frames.
