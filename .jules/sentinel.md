...
## 2026-05-09 - [CRITICAL/HIGH] Fix Null-Pointer Dereference in MPV FFI
**Schwachstelle:** In `crates/vorce-media/src/mpv_decoder.rs` wurden C-Strings aus einem von `libmpv` gelieferten Array ohne Null-Prüfung an `CStr::from_ptr` übergeben. Ein Null-Pointer in diesem Array hätte zu einem sofortigen Absturz (DoS) der Anwendung geführt.
**Lektion:** FFI-Schnittstellen sind inhärent unsicher. Daten vom C-Backend (insbesondere Pointer) müssen vor der Verwendung in Rust zwingend auf Gültigkeit (`.is_null()`) geprüft werden.
**Prävention:** Explizite Null-Pointer-Checks vor jeder Dereferenzierung oder Konvertierung von FFI-Pointern.
