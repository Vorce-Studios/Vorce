...
## 2026-05-31 - [WebSocket JSON Parsing] **Schwachstelle:** Untersuchung eines potenziellen DoS durch malformed JSON in `vorce-control/src/web/websocket.rs`. **Lektion:** Die Analyse ergab, dass die Deserialisierung im Runtime-Pfad (`handle_text_message`) bereits sicher per `map_err` und `? -Operator` behandelt wird. Die gefundenen `unwrap()`-Aufrufe beschränken sich auf Unittests. **Prävention:** Es besteht aktuell kein reproduzierbarer DoS-Vektor in dieser Funktion im Main-Branch. Keine weiteren Codeänderungen erforderlich.
