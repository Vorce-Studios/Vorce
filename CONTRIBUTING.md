...
Thank you for contributing! Vorce is a community-built, maintained and operated project that welcomes contributions from everyone.
...
- If you can reproduce it, try to figure out if it's caused by the video format that you use or by Vorce.
...
- If it looks like it's caused by Vorce (or if you're not sure) and still occurs on the current `main` branch,
  ...
  - In the description, include the following details:
    1. **relevant system information** such as which Vorce version, operating system, Rust version, and graphics card you are using,
...
This can sometimes involve a lot of discussion, as everyone uses Vorce differently.
...
- On the other hand, we are also busy people, often professionally involved in making large
  scale projects ourselves, using Vorce. If you feel like no-one is paying attention to
...
- As with most open source projects, the fastest way to get a feature made is to make it
  ...
  to Vorce, and ask them to make it for you.
...
- Submit from a dedicated branch on your own repository **branched off from current `main`**.
  Your branch should be only about a single topic or area of Vorce.
...
- Submit your pull request to the **`main`** branch of Vorce (which you branched off from).
...
**Note:** Vorce is based on the original MapMap project and maintains its open-source spirit.
