...
function Get-VorceConfigPrompt {
    ...
    # 1. Resolve relative path by searching groups
    $subfolders = @("planning", "monitoring", "audit", "ceo", "deliberation")
    foreach ($sub in $subfolders) {
        $testPath = Join-Path $PromptsDir "$sub/$PromptKey.md"
    ...
    # 2. Fallback to recursive search if not found in standard subfolders
    if ([string]::IsNullOrWhiteSpace($promptTemplate)) {
        $mdFiles = Get-ChildItem -Path $PromptsDir -Filter "$PromptKey.md" -Recurse -File -ErrorAction SilentlyContinue
...
function Get-VorceDashboardDataInstructions {
    $instructionsPath = Join-Path $PSScriptRoot "../../prompts/ceo/dashboard_instructions.md"
    if (Test-Path -LiteralPath $instructionsPath) {
        return (Get-Content -LiteralPath $instructionsPath -Raw -Encoding UTF8).Trim()
    }
    return @"
# Pflicht-Lagebild für diese Entscheidung
