# Vorce Control Protocol (MCP) API

This document describes the Vorce Control Protocol (MCP) integration, which allows external agents (like Jules) to control the Vorce application.

## Overview

The MCP server runs embedded within the Vorce application and communicates via `stdio` using JSON-RPC 2.0. It adheres to the Model Context Protocol standard.
...
* **`vorce://resources/list`**: Lists all available resources.
* **`vorce://app/state`**: Read-only access to the full application state (JSON).
* **`vorce://layers`**: List of layers.
* **`vorce://cues`**: List of cues.
...
To control Vorce from an MCP client:
