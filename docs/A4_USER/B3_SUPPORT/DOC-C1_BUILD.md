...
cargo build --release -p subi --no-default-features --features macos-beta

# Run the application
cargo run --release -p subi --no-default-features --features macos-beta
...
# Linux/macOS: ./target/release/subi
# Windows: .\target\release\subi.exe

# Run release build
./target/release/subi  # Linux/macOS
.\target\release\subi.exe  # Windows
```
