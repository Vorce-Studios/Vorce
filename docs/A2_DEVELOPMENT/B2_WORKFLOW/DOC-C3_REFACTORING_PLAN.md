...
This document serves as the master plan for refactoring the StageGraph application. It is broken down into specific, actionable
...
- **Input**: `crates/stagegraph/src/main.rs` (Settings window block)
- **Actions**:
  1. Create `crates/stagegraph/src/ui/mod.rs` and `crates/stagegraph/src/ui/settings.rs`.
...
- **Validation**:
  - `cargo check -p stagegraph` passes.
...
- **Input**: `crates/stagegraph/src/main.rs` (`egui::SidePanel::left`)
- **Actions**:
  1. Create `crates/stagegraph/src/ui/sidebar.rs`.
...
- **Input**: `crates/stagegraph/src/main.rs` (`egui::BottomPanel`)
- **Actions**:
  1. Create `crates/stagegraph/src/ui/timeline.rs`.
...
- **Input**: `crates/stagegraph/src/main.rs` (`egui::CentralPanel`)
- **Actions**:
  1. Create `crates/stagegraph/src/ui/canvas.rs`.
...
- **Input**: `crates/stagegraph/src/main.rs` (`struct App`)
- **Actions**:
  1. Create `crates/stagegraph/src/state.rs`.
...
- **Input**: `crates/stagegraph/src/main.rs` (`impl ApplicationHandler for App`)
- **Actions**:
  1. Create `crates/stagegraph/src/inputs.rs`.
...
- **Input**: `crates/stagegraph/src/main.rs` (`App::update` method)
- **Actions**:
  1. Create `crates/stagegraph/src/systems/audio.rs` and `crates/stagegraph/src/systems/physics.rs`.
...
- **Input**: `crates/stagegraph/src/main.rs` (`App::render` WGPU sections)
- **Actions**:
  1. Create `crates/stagegraph/src/renderer.rs`.
