...
- **Files**: `crates/stagegraph-core/src/module.rs` -> `crates/stagegraph-core/src/module/mod.rs` + `trigger.rs`
- **Output**:
    - Directory `crates/stagegraph-core/src/module/` created.
    - File `crates/stagegraph-core/src/module.rs` moved to `mod.rs`.
    - `TriggerConfig`, `TriggerTarget`, `TriggerMappingMode` moved to `module/trigger.rs`.
    - `mod.rs` re-exports everything publicly.
- **Verification**: `cargo check -p stagegraph-core`

#### [Task 4.1.2] Extract ModulePart
- **Input**: `crates/stagegraph-core/src/module/mod.rs`
- **Output**:
    - `ModulePart` struct and its impl blocks moved to `crates/stagegraph-core/src/module/part.rs`.
    - `mod.rs` imports and re-exports `ModulePart`.
- **Verification**: `cargo check -p stagegraph-core`, `cargo test -p stagegraph-core`
...
- **Files**: `crates/stagegraph-ui/src/editors/module_canvas.rs` -> `crates/stagegraph-ui/src/editors/module_canvas/mod.rs` + `types.rs`
- **Output**:
    - Directory `crates/stagegraph-ui/src/editors/module_canvas/` created.
    - File moved to `mod.rs`.
    - `MyDataType`, `MyValueType`, `MyResponse`, `MyNodeTemplate` moved to `types.rs`.
- **Verification**: `cargo check -p stagegraph-ui`

#### [Task 4.2.2] Extract Node Registry & Base Traits
- **Input**: `crates/stagegraph-ui/src/editors/module_canvas/mod.rs`
- **Output**:
    - Create `nodes/mod.rs`.
...
- **Verification**: `cargo check -p stagegraph-ui`, Manual check of Media Nodes.
...
- **Verification**: `cargo check -p stagegraph-ui`, Manual check of Effect Nodes.
...
- **Verification**: `cargo check -p stagegraph-ui`, Full manual regression test.
