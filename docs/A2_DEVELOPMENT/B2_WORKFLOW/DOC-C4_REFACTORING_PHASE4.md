...
- **Files**: `crates/subi-core/src/module.rs` -> `crates/subi-core/src/module/mod.rs` + `trigger.rs`
- **Output**:
    - Directory `crates/subi-core/src/module/` created.
    - File `crates/subi-core/src/module.rs` moved to `mod.rs`.
    - `TriggerConfig`, `TriggerTarget`, `TriggerMappingMode` moved to `module/trigger.rs`.
    - `mod.rs` re-exports everything publicly.
- **Verification**: `cargo check -p subi-core`

#### [Task 4.1.2] Extract ModulePart
- **Input**: `crates/subi-core/src/module/mod.rs`
- **Output**:
    - `ModulePart` struct and its impl blocks moved to `crates/subi-core/src/module/part.rs`.
    - `mod.rs` imports and re-exports `ModulePart`.
- **Verification**: `cargo check -p subi-core`, `cargo test -p subi-core`
...
- **Files**: `crates/subi-ui/src/editors/module_canvas.rs` -> `crates/subi-ui/src/editors/module_canvas/mod.rs` + `types.rs`
- **Output**:
    - Directory `crates/subi-ui/src/editors/module_canvas/` created.
    - File moved to `mod.rs`.
    - `MyDataType`, `MyValueType`, `MyResponse`, `MyNodeTemplate` moved to `types.rs`.
- **Verification**: `cargo check -p subi-ui`

#### [Task 4.2.2] Extract Node Registry & Base Traits
- **Input**: `crates/subi-ui/src/editors/module_canvas/mod.rs`
- **Output**:
    - Create `nodes/mod.rs`.
...
- **Verification**: `cargo check -p subi-ui`, Manual check of Media Nodes.
...
- **Verification**: `cargo check -p subi-ui`, Manual check of Effect Nodes.
...
- **Verification**: `cargo check -p subi-ui`, Full manual regression test.
