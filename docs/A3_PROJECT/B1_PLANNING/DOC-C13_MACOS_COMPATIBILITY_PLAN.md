...
- review `subi-bevy` feature flags
...
Deliverables:
- `cargo build -p subi` works on macOS
...
- [ ] Validate `crates/subi-bevy/Cargo.toml` feature set on macOS
- [ ] Verify `cargo build --release -p subi` on Apple Silicon
- [ ] Verify `cargo build --release -p subi` on Intel macOS
...
- [ ] Implement real `subi-io` Syphon support
...
- Primary files:
  - `Cargo.toml`
  - `crates/subi/Cargo.toml`
  - `crates/subi-bevy/Cargo.toml`
...
- Definition of done:
  - `cargo build -p subi` is expected to work on macOS
...
- Primary files:
  ...
  - `crates/subi/src/orchestration/outputs.rs`
...
- Primary files:
  - `crates/subi-io/src/syphon/mod.rs`
  - `crates/subi-io/src/virtual_camera/mod.rs`
