...
- **Spout Support**: Muss für wgpu 0.19+ (modernisiert auf DX11/DX12 interop) aktualisiert werden. (`crates/stagegraph-render/src/spout.rs`) (Status: 🔴)
- **HAP Q Alpha**: Handling für sekundäre Texturen in `hap_decoder.rs` implementieren. (Status: 🔴)
- **About Dialog & Export**: Grundlegende Implementierung für Release-Build erforderlich. (`crates/stagegraph/src/app/actions.rs`) (Status: 🔴)
...
| :--- | :--- | :---: | :--- |
| **stagegraph-core** | TODO Resolution & Graph Validation | ✅ | Zyklus-Check & Auto-Disconnect implementiert. Clippy sauber. |
| **stagegraph-io** | NDI Activation & Test Stability | ✅ | NDI-Warnungen entfernt, Projekt-Roundtrip verifiziert. |
| **stagegraph-render** | Paint Image Loading | ✅ | Synchrones Laden von Bildern in GPU-Cache implementiert. |
| **stagegraph-ui** | ID Sync & Clippy Scrub | ✅ | Node-Editor ID-Synchronisation gefixt. Redundante Bindings entfernt. |
| **stagegraph-bevy** | Integration Audit | ✅ | Main-Loop und UI-Brücke validiert. |
| **stagegraph-mcp** | Protocol Audit | ✅ | JSON-Schnittstellen für AI-Tools stabilisiert. |
