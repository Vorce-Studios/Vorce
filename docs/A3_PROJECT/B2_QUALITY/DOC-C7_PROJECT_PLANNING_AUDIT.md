...
- **Code:** `subi-io/src/ndi` existiert, enthält aber "TODO: Implement actual frame sending".
- **Fazit:** Die Implementierung hinkt der Planung hinterher. NDI ist nur ein Skelett.

#### 2. MCP Integration
- **Roadmap:** "MCP-API-Referenz (TODO)".
- **Code:** `subi-mcp` Crate existiert und scheint funktional. Die Dokumentation fehlt aber.
...
- Ist `subi-control/src/web` (WebSocket) wirklich "NICHT NUTZEN"? Wenn ja, sollte der Code als `deprecated` markiert oder entfernt werden.
