...
- `SUBI_ENABLE_SELF_HOSTED_POST_MERGE=true`
...
- `SUBI_SELF_HOSTED_RUN_IGNORED_GPU_TESTS=true`
- `SUBI_SELF_HOSTED_RUN_VISUAL_AUTOMATION=true`
...
1. Repo-Variable `SUBI_ENABLE_SELF_HOSTED_POST_MERGE` auf `false` setzen oder ganz entfernen
...
- global aus: Repo-Variable deaktivieren (`SUBI_ENABLE_SELF_HOSTED_POST_MERGE=false`)
...
- Release-Build von `subi` mit `audio,ffmpeg`
...
Relative Pfade fuer `SUBI_VISUAL_CAPTURE_OUTPUT_DIR` werden gegen die Repo-Wurzel aufgeloest.
